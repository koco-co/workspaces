...
- 如编排器提供了 `source_context`，必须先确认其中声明的 repo/branch 已生效，再开始阅读源码。
- DTStack 不允许仅依据蓝湖原文直接写用例；应优先读取正式需求文档 / 增强版 PRD，再用源码补齐真实按钮、字段、表名、规则分支。
- 前置条件需补充真实执行上下文（数据源类型、schema/table、关键字段、必要时的建表或准备数据说明）。
- 表单类步骤优先使用结构化块，明确按钮名称与字段填写内容。
...
- `新建多表规则，在“校验类型”选择“一致性校验”，在“选择校验字段”勾选“order_amount”“discount_amount”`
- `填写相关信息后提交`
