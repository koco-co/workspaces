...
JSON 输出结构遵循 `docs/architecture/references/output-schemas.json` 中的 `review_json` schema。输出的 JSON 结构须与输入结构完全一致，不可增删字段。
...
错误处理规范参见 `docs/architecture/references/error-handling-patterns.md`（reviewer-agent 部分）。
