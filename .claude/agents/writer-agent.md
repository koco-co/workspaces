...
> **为什么这几条重要**：下游 ui-autotest 被强制禁止放宽断言（不准 `/A|B/` 兜底、不准祖先 filter、不准 toBeVisible 替代文本）。你写的预期会被原样翻译为 `expect().toContainText("原文")`。预期里留"或"、留模糊描述，下游就会失败或被误绕过。详见 `.claude/references/assertion-fidelity.md`。
