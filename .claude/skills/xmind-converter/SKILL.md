...
- **用途**：将中间 JSON 测试用例数据转换为可直接打开的 `.xmind` 文件，并刷新仓库根目录 `latest-output.xmind` 快捷入口。
...
- `latest-output.xmind` 是统一主验收入口；真实输出路径仍按模块与版本落盘。
...
| 输入类型 | 示例 | 说明 |
| --- | --- | --- |
| 单个 JSON 文件 | `cases/requirements/orders/v2.0/temp/final-reviewed.json` | 直接转换为一个 `.xmind` |
| 多个 JSON 文件 | `input1.json input2.json` | 合并后输出；`meta` 取首个文件，L1 按需求拆分 |
| `temp/` 目录 | `cases/requirements/orders/v2.0/temp/` | 自动扫描目录内全部 `.json` |
...
| 输出类型 | 路径规则 | 说明 |
| --- | --- | --- |
| PRD 级 XMind | `cases/xmind/${module_key}/v${version}/<功能名>.xmind` 或无版本目录 | 默认输出粒度 |
| Story 聚合 XMind | `cases/xmind/${module_key}/v${version}/Story-YYYYMMDD.xmind` | 多 PRD 追加到同一文件时使用 |
| 快捷入口 | `latest-output.xmind` | 始终指向本次最新真实输出 |
...
| 4 | 判断写入模式 | 新建 / `--append` / `--replace` 三选一；Story 聚合优先追加，模块重跑优先替换 |
| 5 | 执行转换 | 调用 `json-to-xmind.mjs` 生成或更新 `.xmind`，并刷新 `latest-output.xmind` |
...
- 执行前必须阅读：本文件、`rules/xmind-output.md`、`references/xmind-structure-spec.md`、`.claude/skills/xmind-converter/scripts/json-to-xmind.mjs`。
- 所有路径必须是绝对路径或相对于 qa-flow 根目录的有效路径。
- Story 聚合场景优先使用 `--append`，模块级重跑场景优先使用 `--replace`；普通单需求输出保持新建模式。
- 输出路径必须遵守 XMind naming contract，不得把真实产物写成保留文件名 `latest-output.xmind`。
...
2. 目标 `.xmind` 已落盘到 canonical 目录，并刷新 `latest-output.xmind`。
...
- `rules/xmind-output.md`：XMind 命名 contract、路径规则、追加 / 替换口径
