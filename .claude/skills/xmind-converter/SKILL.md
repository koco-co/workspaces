...
> `meta.module_key` 优先用于确定模块路径；`meta.prd_version` 决定是否创建版本子目录。
...
**追加模式行为：**
- 目标文件的 rootTopic title 与根节点标题（`${displayName}${version}迭代用例` 或 `${displayName}`）相同 → 将新 L1 节点追加到该 root 的 children
- 不匹配 → 作为新 sheet 添加
...
验证通过标准：
- 文件大小 > 1KB
- `rootTopic.title` 等于根节点标题：
  - 有 trackerId：`${displayName}${version}迭代用例(#${trackerId})`
  - 无 trackerId（有 version）：`${displayName}${version}迭代用例`
  - 无 version 无 trackerId：`${displayName}` 或 `${project_name}`
...
文件结构摘要：
- 根节点：<displayName>[<version>迭代用例[(#<trackerId>)]]
- 版本：<version>
- 需求：<requirement_name>
- 模块数：<N> 个
- 总用例数：<M> 条
