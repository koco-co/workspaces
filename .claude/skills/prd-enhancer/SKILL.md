...
> **仅在独立调用（用户直接触发 prd-enhancer）时执行。** 若 prd-enhancer 是由 `test-case-generator` 流水线自动调用的，跳过本步骤（req-elicit 已在更早阶段执行）。

**触发判断：**

若 PRD frontmatter `status` 已为 `elicited` / `formalized` / `enhanced`，或 PRD 末尾已存在 `## 需求澄清结果` 章节，跳过本步骤。

**执行流程：**

1. 读取 PRD 文件，按 Tier 1 三个维度快速评估可测试性（参考 `test-case-generator/references/elicitation-dimensions.md`）：
   - **字段定义**（`field_definition`）：是否有字段名+类型+必填+枚举值的完整表格
   - **页面清单**（`page_inventory`）：是否能识别出所有目标页面（列表/新增/编辑/详情/弹窗）
   - **验收标准**（`io_criteria`）：是否有成功/失败场景的预期结果描述

2. 计算加权评分（字段定义 3x，页面清单 2x，验收标准 2x）：
   - **>= 90%**：自动跳过澄清，直接进入 Step 0.5
   - **< 90%**：提出最多 3 个问题

3. **逐页推进**：先展示问题总数，再每次只展示 1 个问题，等用户回应后推进下一条：

```
需补充 {N} 项信息，逐条提问中。
  全部推荐 — 接受所有 AI 推荐，一步完成
  直接开始 — 跳过全部，继续流程
```
...
4. 所有页完成后将结果追加到 PRD 末尾的 `## 需求澄清结果` 章节，frontmatter `status` 更新为 `elicited`。

**用户跳过处理：**

| 用户回复 | 行为 |
|---------|------|
| 单字母 `A`/`B`/`C` | 记录当前页选项，推进下一页 |
| 「全部推荐」 | 所有剩余页均选 A，直接进入 Step 0.5 |
| 「直接开始」 | 跳过全部剩余页，进入 Step 0.5 |
| 「跳过」/`S` | 仅跳过当前页（标记 inferred），推进下一页 |
...
| 字段名称 | 来源 | 说明 |
|---------|------|------|
| 状态 | 用户确认 | 枚举值：待处理 / 处理中 / 已完成 |

### 验收标准

| 场景类型 | 场景描述 | 预期结果 | 来源 |
|---------|---------|---------|------|
| 正常 | 填写完整表单后点击【提交】 | 页面提示"新增成功"，列表新增一条记录 | 用户确认 |

<!-- elicitation-status: completed | round: 1 | questions: 2/3 -->
```
