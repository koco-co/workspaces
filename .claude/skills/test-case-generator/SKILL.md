...
6. **Writer 步骤**：
   - **6a. 源码预提取（config.repos 非空时，编排器执行）**：在启动 Writer Agent 前，若 config.repos 非空，编排器必须从 `.repos/` 中预提取以下关键信息，格式化后填入 Writer prompt 的「编排器预提取的关键信息」占位符：
     - **后端源码**（`source_context.backend` 指定的仓库）：
     ...
     - **前端源码**（`source_context.frontend` 指定的仓库）：
       4. 菜单导航：Grep `menuConfig|routes|path:|menu` + 模块关键词，确认实际菜单层级和路由路径
       5. 按钮文案：Grep `Button|<button|onClick|btn` + 页面关键词，提取按钮显示文本（如「新增商品」「批量导出」「保存」）
