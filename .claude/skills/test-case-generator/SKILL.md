...
| 4.5 | `source-analyze` | 执行 | 并行双 Agent 提取前端/后端源码上下文，输出 source-context.md（config.repos 为空时跳过） |
...
| 4.5 | source-analyze | `step-source-analyze.md` | 执行 | 并行双 Agent 提取前端/后端源码上下文，输出 source-context.md（config.repos 为空时跳过） |
...
6. **Writer 步骤**：
   - **6a. 注入 source-context.md 路径**：从 `.qa-state.json` 的 `source_context_file` 字段读取路径（step source-analyze 已写入），填入 Writer prompt 的「源码上下文文件路径」占位符。若该字段为空（source-analyze 被跳过或失败），填入「无」，Writer 降级为按 PRD 原文编写
