...
description: QA 测试用例生成主编排 Skill。一键触发完整自动化流程：PRD 增强 → 健康度预检 → Brainstorming + 解耦分析 → Checklist 预览 → 并行生成用例 → 评审 → 输出 XMind。当用户说「生成测试用例」「根据需求文档生成用例」「为 Story-xxx 生成用例」时触发。支持 --quick 快速模式和断点续传。
---
...
- 全局 `brainstorming` — 测试范围确认（Step 3，--quick 时跳过）
