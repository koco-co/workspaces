...
| `source_context_file` | source-analyze 步骤（Step 4.5）输出的源码上下文文件路径（如 `cases/prds/202604/temp/source-context.md`）；config.repos 为空或分析失败时为空字符串 `""`；Writer/Reviewer 通过此字段定位预提取的前端/后端信息，无需重复 grep 源码 |
