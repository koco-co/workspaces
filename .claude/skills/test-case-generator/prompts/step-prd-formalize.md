...
- 出现冲突时，根据 `qa-state.feature_type` 决定处理方式：

  | 需求类型 | PRD 与源码冲突时的处理 |
  |---------|----------------------|
  | `new-feature` | 优先采信 PRD；源码作为逻辑参考，标注"基于源码推断"后保留双方描述 |
  | `partial-update` | **源码优先**；PRD 描述若与源码不符，以源码为准；在冲突字段旁标注"（PRD原文为xxx，以源码实现为准）" |

- `partial-update` 场景的额外要求：
  - 按钮名、字段名、菜单路径必须从 source-context.md 中提取，禁止直接用 PRD 原文描述
  - 若 PRD 图片中识别到的按钮/字段名与源码不一致，**不得将 PRD 图片内容写入正式文档**；在对应位置注明「PRD 原图中显示为「xxx」，源码实现为「yyy」，本文档以源码为准」
- 确保正式文档可独立阅读，不依赖蓝湖原文
