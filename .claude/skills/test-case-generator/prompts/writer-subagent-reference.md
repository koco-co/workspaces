...
```json
{ "step": "在「统计函数」下拉框选择「key范围校验」", "expected": "..." },
{ "step": "在「字段」下拉框选择「json_field」字段", "expected": "..." },
{ "step": "在「校验方法」下拉框选择「包含」", "expected": "..." }
```

**例外情况（允许单独步骤）：**
- 字段选择触发页面联动，且联动结果是本用例的核心验证点
- 该字段本身的选项列表就是被测内容（如验证下拉框选项仅含「包含」「不包含」）
