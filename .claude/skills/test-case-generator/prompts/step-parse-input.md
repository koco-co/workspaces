...
5.1. **模块确认（必须交互）**
   ...
   **PRD front-matter Schema（蓝湖导入时填写）：**
   ...
   - `prd_id`：从 URL 参数 `docId` 提取（取完整值，可能是 UUID 或数字）
   - `prd_name`：**只保留功能名称部分**，去掉序号和项目版本前缀。例如蓝湖文档名为「数据资产V6.4.10 - 15525【内置规则丰富】一致性校验」，则 `prd_name` 应提取为「【内置规则丰富】一致性校验」（即从第一个【开始到末尾，或去掉数字/版本前缀后的内容）
