...
```
 workspace/{{project}}/features/{{ym}}-{{slug}}/cases.xmind
```

- `{{ym}}`：产物生成的年月（如 `202604`）
- `{{slug}}`：需求标识（来源于 `meta.requirement_name` 或 PRD 文件名 slugified）
...
输出路径: workspace/{{project}}/features/{{ym}}-{{slug}}/archive.md
```
...
```
✅ XMind 已生成：{{project_root}}/workspace/{{project}}/features/{{ym}}-{{slug}}/cases.xmind
```
