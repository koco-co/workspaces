...
```
workspace/{project}/features/{ym}-{slug}/
├── enhanced.md              # 主文档（本模板）
├── prd.md                   # 蓝湖导出的原始 PRD（probe 产物）
...
project: dataAssets
prd_slug: xxx
prd_dir: workspace/dataAssets/features/202604-xxx/
...
| `prd_dir`            | CLI（init）                            | `workspace/{project}/features/{ym}-{slug}/`                      |
...
（由主 agent 视觉能力识别图像语义 + source-scanner-agent 的源码事实补充）
...
（由 source-scanner-agent 扫描产出；`discuss read` 默认自动 deref 外溢 blob）
...
主 agent 在 §3 为每张图片生成识别摘要（blockquote），格式：
...
source-scanner-agent 扫描后应覆盖 W001 / W002 / W004 / W005 / W007 / W008；W003 / W006 在 3.6 维度扫描阶段补齐。
