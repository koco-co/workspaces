...
| `prd_path`           | `string` | 否   | 关联 PRD 文件的相对路径，如 `"workspace/{{project}}/features/{{ym}}-{{slug}}/prd.md"` |

### meta 示例

> 以下示例数据来自实际项目「数据资产平台」，新项目应将 `project_name`、`module_key` 等字段替换为本项目对应值。
...
```json
{
  "meta": {
    ...
    "prd_path": "workspace/{{project}}/features/202604-shangpinguanli/prd.md"
  }
}
```
...
| 字段       | 类型     | 必填 | 说明                                                                                                                 |
| ---------- | -------- | ---- | -------------------------------------------------------------------------------------------------------------------- |
| `step`     | `string` | 是   | 操作步骤描述，具体可执行。涉及表单填写时**必须使用 `\n` 换行 + `- 字段: 值` 列表格式**，禁止将所有字段用逗号堆在一行 |
| `expected` | `string` | 是   | 预期结果描述，精确可观测                                                                                             |

### step 字段多行格式说明

当步骤涉及表单填写时，`step` 字段必须包含 `\n` 换行符，使用 `- *字段: 值` 列表格式逐字段列举。示例：
