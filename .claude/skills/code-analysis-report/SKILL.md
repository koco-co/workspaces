...
description: 测试工程师专用的代码分析报告工作流 Skill。当用户粘贴后端报错日志、curl 请求信息、Jenkins 合并冲突信息，或提到「帮我分析这个报错」「生成 bug 报告」「分析一下这个错误」「分析一下冲突」「看看这个异常」时，必须触发此 Skill。支持两种模式：(1) Bug 分析 → 生成 HTML 格式报告存入 Code分析报告/bugs/ 目录；(2) Jenkins 合并冲突分析 → 生成 HTML 报告存入 Code分析报告/conflicts/ 目录。所有模式在开始分析前必须确认代码分支并自动拉取。输出精美 HTML 格式，专为禅道富文本编辑器粘贴使用。
---
...
| 报错特征 | 目标仓库 |
|---------|---------|
| `com.dtstack.center.assets` / `dt-center-assets` | `gitlab-projects/dt-insight-web/dt-center-assets/` |
| `com.dtstack.center.metadata` / `dt-center-metadata` | `gitlab-projects/dt-insight-web/dt-center-metadata/` |
| `com.dtstack.dagschedulex` / `DAGScheduleX` | `gitlab-projects/dt-insight-plat/DAGScheduleX/` |
| `com.dtstack.datasource` / `datasourcex` | `gitlab-projects/dt-insight-plat/datasourcex/` |
| `com.dtstack.ide` / `dt-center-ide` | `gitlab-projects/dt-insight-plat/dt-center-ide/` |
| `com.dtstack.public.service` / `dt-public-service` | `gitlab-projects/dt-insight-plat/dt-public-service/` |
| `com.dtstack.sql.parser` / `SQLParser` | `gitlab-projects/dt-insight-plat/SQLParser/` |
| `com.dtstack.engine` / `engine-plugins` | `gitlab-projects/dt-insight-engine/engine-plugins/` |
| 前端报错 / `dt-insight-studio` | `gitlab-projects/dt-insight-front/dt-insight-studio/` |
| 定制项目（信永中和等） | `gitlab-projects/CustomItem/<对应仓库>/` |
...
> **安全规则**：对 gitlab-projects 下的仓库，只允许 `fetch`、`pull`、`checkout` 操作。严禁 `push`、`commit`、修改任何源码文件。详见 CLAUDE.md「源码仓库安全规则」。
...
```bash
TODAY=$(date +%Y-%m-%d)
REPORT_DIR="<项目根>/Code分析报告/bugs/$TODAY"
mkdir -p "$REPORT_DIR"
...
**写入完成后输出：**
```
[v] Bug 报告已生成：Code分析报告/bugs/{日期}/{文件名}.html
...
**存储路径：**`Code分析报告/conflicts/${yyyy-MM-dd}/<冲突描述>.html`
...
| 模式 | 文件格式 | 存储位置 |
|------|---------|---------|
| Bug 分析 | `<Bug标题>.html` | `Code分析报告/bugs/{yyyy-MM-dd}/` |
| 合并冲突 | `<冲突描述>.html` | `Code分析报告/conflicts/{yyyy-MM-dd}/` |
...
| 代码仓库 | 使用 `gitlab-projects/` 下的源码仓库，详见 CLAUDE.md「源码仓库详细清单」 |
