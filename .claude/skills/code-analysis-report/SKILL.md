...
description: 测试工程师专用的代码分析与 Hotfix 用例生成 Skill。当用户粘贴前端或后端报错日志、curl 请求信息、Jenkins 合并冲突信息，或提到「帮我分析这个报错」「生成 bug 报告」「分析一下这个错误」「分析一下冲突」「看看这个异常」时，必须触发此 Skill。还支持 Hotfix 分支用例生成：当用户输入「应用: org/repo, 版本: hotfix_x.x.x_bugId」时，必须直接进入模式E，禁止展示模式选择菜单、禁止向用户提问，并立即拉取后端 hotfix 分支。支持四种执行模式与一种补料门禁：后端 Bug 分析、Jenkins 合并冲突分析、前端报错分析、模式D 信息不足补料、Hotfix 用例生成。A/B/C 产出 HTML 报告，E 产出 `cases/archive/online-cases/` 下的测试用例；所有需要读源码的模式在开始前都必须确认代码分支并自动拉取，其中模式E按 Hotfix 输入直接自动拉取。
---
...
- **Hotfix 用例生成（模式E）**：输入格式为 `应用: {org}/{repo}, 版本: hotfix_{version}_{bugId}`，直接触发，无需选择菜单，不向用户提问；固定顺序为先拉取后端 hotfix 分支，再分析代码变更，再确定前端仓库和分支，最后非阻塞尝试获取禅道信息。
...
> **若用户输入同时满足以下两个条件：**
> - 包含 `应用:` 关键词，后跟 `{org}/{repo}` 格式
> - 包含 `版本: hotfix_` 关键词，后跟版本号和 Bug ID
>
> → **立即跳转 Mode E**
> → **禁止展示模式选择菜单**
> → **禁止向用户询问任何问题，直接开始拉取后端 hotfix 分支**
>
> 示例输入（直接触发，无需确认）：`应用: dt-insight-web/dt-center-assets, 版本: hotfix_6.2.x_145513`

---

| 模式 | 触发信号 | 执行路径 |
| --- | --- | --- |
| **模式A：后端 Bug 分析** | 含 `Exception` / `Caused by` / `at com.` / `java.lang` 等 Java 堆栈特征，或后端 curl 报错 | `.claude/skills/code-analysis-report/references/backend-analysis-flow.md` |
| **模式B：合并冲突** | 含 `<<<<<<< HEAD` / `=======` / `>>>>>>>` 标记，或提到 Jenkins 冲突 | `.claude/skills/code-analysis-report/references/conflict-analysis-flow.md` |
| **模式C：前端报错分析** | 含 `TypeError` / `ReferenceError` / `[Vue warn]` / `React` / `Uncaught Error` / `error TS` / `Hydration failed` 等前端错误关键词 | `.claude/skills/code-analysis-report/references/frontend-analysis-flow.md` |
| **模式D：信息不足** | 描述模糊，缺少日志、冲突内容、仓库信息或 Hotfix 输入关键信息 | 输出补料清单并暂停 |
| **模式E：Hotfix 用例生成** | 输入包含 `应用: {org}/{repo}, 版本: hotfix_{version}_{bugId}` 格式 | `.claude/skills/code-analysis-report/references/hotfix-case-flow.md` |
...
- `.claude/skills/code-analysis-report/references/hotfix-case-writing.md`：模式 E Hotfix 用例编写规范（标题、frontmatter、预期结果、自审清单）
- `.claude/skills/code-analysis-report/references/bug-report-template.md`：后端 / 前端 Bug HTML 报告 JSON Schema
