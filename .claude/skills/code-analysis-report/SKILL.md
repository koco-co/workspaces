...
| 模式E | 1) 解析 Hotfix 输入 → 2) 拉取后端 hotfix 分支 → 3) 分析 Hotfix 代码变更 → 4) 确定前端仓库和分支 → 5) 非阻塞尝试获取禅道 Bug → 6) 参考历史用例 → 7) 生成一条测试用例（标题使用 `【{zentao_bug_id}】验证xxx`，并遵循 `references/hotfix-case-writing.md`）→ 8) 写入 `cases/archive/online-cases/` 并创建快捷链接 → 9) 完成报告 → 10) 自审清单 | `references/hotfix-case-flow.md` |
...
- 模式 E 生成用例时必须同时遵循 `references/hotfix-case-flow.md` 与 `references/hotfix-case-writing.md`；标题必须使用 `【{zentao_bug_id}】验证xxx`，禁止使用 `【P0/P1/P2】`。
...
- `references/hotfix-case-writing.md`：模式 E Hotfix 用例编写规范（标题、frontmatter、预期结果、自审清单）
