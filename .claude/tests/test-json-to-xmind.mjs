...
await runTest("config 驱动输出根节点与 L1 元数据（原 DTStack 样例）", async () => {
  ...
  assert(result.code === 0, "config 驱动 XMind 生成成功");
  assert(existsSync(dtOutput), "config 驱动 XMind 文件已生成");
  ...
  // 新行为：root title 使用 config.project.displayName + 版本（而非 project_name），
  // 当 module_key 对应模块不在 config.modules 中时
  assert(root?.title?.includes("v6.4.10迭代用例"), "root title 包含版本和迭代用例");
  // L1 title 统一使用 【version】name(#ticket) 格式
  assert(
    l1?.title === "【v6.4.10】【内置规则丰富】质量校验不通过时支持发送邮件(#10307)",
    "L1 title 使用通用格式 【version】name(#ticket)",
  );
  assert(
    Array.isArray(l1?.labels) && l1.labels.includes("(#15602)"),
    "L1 labels 包含 requirement_id",
  );
  assert(
...
await runTest("多输入 DTStack JSON 会在同一 root 下生成多个 L1 节点", async () => {
  ...
  assert(root?.title?.includes("v6.4.10迭代用例"), "多输入 root title 包含版本和迭代用例");
  assert(l1Nodes.length === 2, "多输入输出包含 2 个 L1 节点");
  assert(
    l1Titles.includes("【v6.4.10】【内置规则丰富】质量校验不通过时支持发送邮件(#10307)") &&
      l1Titles.includes("【v6.4.10】【内置规则丰富】一致性，多表数据一致性比对(#15525)"),
    "多输入输出保留每个 requirement 的独立 L1 title（通用格式）",
  );
