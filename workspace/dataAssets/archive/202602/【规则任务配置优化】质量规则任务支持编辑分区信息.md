---
suite_name: "【规则任务配置优化】质量规则任务支持编辑分区信息"
description: "【规则任务配置优化】质量规则任务支持编辑分区信息用例归档"
tags:
  - "运行"
  - "生成"
  - "二级分区"
...
| 1 | 进入【数据资产】-【数据质量】-【规则任务管理】页面 | 进入成功 |
| 2 | 新建监控规则, 配置如下: | 配置成功 |
| 3 | 监控对象: - 规则名称: rule01 - 数据表: dwd_voyah_vehicle_sales_dates - 选择动态分区: factory_date/sale_date = bdp.system.bizdate | 运行成功 |
| 4 | 监控规则: 完整性校验: - 生效范围: 字段级 - 字段: final_price - 统计函数: 字段取值校验 - 期望值: >= 0 | 校验结果: 校验通过 |
| 5 | 调度属性: - 调度周期: 时 - 生效日期: T~T+1 - 间隔时间: 1小时 - 其它默认 - 实例生成方式: 「立即生成」 | 数据预览成功: 3条数据 |
| 6 | 运行规则任务rule01, 立即执行 | 1) 任务运行成功 |
| 7 | 进入【校验结果查询】, 检查实例详情页面 | 规则任务rule01详情页中, 分区配置变更成功 |
| 8 | 进入【规则任务管理】, 编辑rule01, 修改分区: 选择已有分区: factory_date=20260115 , 点击数据预览 | 实例停止运行, 实例状态依次变更: 【停止中】、【中途停止】 |
| 9 | 后面配置不变, 保存后再次运行任务 | 分区配置变更成功, 变更范围: |
| 10 | 进入【校验结果查询】, 检查【运行中、等待运行】的rule01实例的详情页 | 表名悬浮提示中的分区配置信息 |
| 11 | 检查后续生成的rule01实例的详情页 | 规则详情页中, 表级报告中的表名和分区信息 后续生成的实例正常运行, 校验结果: 校验通过 |
...
| 1 | 进入【数据资产】-【数据质量】-【规则任务管理】页面 | 进入成功 |
| 2 | 新建监控规则, 配置如下: | 配置成功 |
| 3 | 监控对象: - 规则名称: rule01 - 数据表: dwd_voyah_vehicle_sales_dates - 手动输入分区: factory_date=20260202/sale_date=20260202 | 运行成功 |
| 4 | 监控规则: 完整性校验: - 生效范围: 字段级 - 字段: final_price - 统计函数: 字段取值校验 - 期望值: >= 0 | 校验结果: 校验不通过 |
| 5 | 调度属性: - 调度周期: 时 - 生效日期: T~T+1 - 间隔时间: 1小时 - 其它默认 - 实例生成方式: 「立即生成」 | 数据预览成功: 暂无数据 |
| 6 | 运行规则任务rule01, 立即执行 | 1) 任务运行成功2) 规则任务rule01详情页中, 分区配置变更成功 |
| 7 | 进入【校验结果查询】, 检查实例详情页面 | 实例停止运行, 实例状态依次变更: 【停止中】、【中途停止】 |
| 8 | 进入【规则任务管理】, 编辑rule01, 修改分区: 选择动态分区: factory_date/sale_date = bdp.system.bizdate , 点击数据预览 | 分区配置变更成功, 变更范围: |
| 9 | 后面配置不变, 保存后再次运行任务 | 表名悬浮提示中的分区配置信息 |
| 10 | 进入【校验结果查询】, 检查【运行中、等待运行】的rule01实例的详情页 | 规则详情页中, 表级报告中的表名和分区信息 后续生成的实例正常运行, 校验结果: 校验通过 |
| 11 | 检查后续生成的rule01实例的详情页 |  |
...
| 1 | 进入【数据资产】-【数据质量】-【规则任务管理】页面 | 进入成功 |
| 2 | 新建监控规则, 配置如下: | 配置成功 |
| 3 | 监控对象: - 规则名称: rule01 - 数据表: dwd_voyah_vehicle_sales_dates - 选择已有分区: factory_date=20260202/sale_date=20260202 | 运行成功 |
| 4 | 监控规则: 完整性校验: - 生效范围: 字段级 - 字段: final_price - 统计函数: 字段取值校验 - 期望值: >= 0 | 校验结果: 校验不通过 |
| 5 | 调度属性: - 调度周期: 时 - 生效日期: T~T+1 - 间隔时间: 1小时 - 其它默认 - 实例生成方式: 「立即生成」 | 数据预览成功: 3条数据 |
| 6 | 运行规则任务rule01, 立即执行 | 1) 任务运行成功2) 规则任务rule01详情页中, 分区配置变更成功 |
| 7 | 进入【校验结果查询】, 检查实例详情页面 | 实例停止运行, 实例状态依次变更: 【停止中】、【中途停止】 |
| 8 | 进入【规则任务管理】, 编辑rule01, 修改分区: 手动输入分区: factory_date=20260115/sale_date=20260201 , 点击数据预览 | 分区配置变更成功, 变更范围: |
| 9 | 后面配置不变, 保存后再次运行任务 | 表名悬浮提示中的分区配置信息 |
| 10 | 进入【校验结果查询】, 检查【运行中、等待运行】的rule01实例的详情页 | 规则详情页中, 表级报告中的表名和分区信息 后续生成的实例正常运行, 校验结果: 校验通过 |
| 11 | 检查后续生成的rule01实例的详情页 |  |
...
| 1 | 进入【数据资产】-【数据质量】-【规则任务管理】页面 | 进入成功 |
| 2 | 新建监控规则, 配置如下: | 配置成功 |
| 3 | 监控对象: - 规则名称: rule01 - 数据表: dwd_voyah_vehicle_sales_dates - 手动输入分区: factory_date=20260202 | 运行成功 |
| 4 | 监控规则: 完整性校验: - 生效范围: 字段级 - 字段: final_price - 统计函数: 字段取值校验 - 期望值: >= 0 | 校验结果: 校验不通过 |
| 5 | 调度属性: - 调度周期: 时 - 生效日期: T~T+1 - 间隔时间: 1小时 - 其它默认 - 实例生成方式: 「立即生成」 | 数据预览成功: 3条数据 |
| 6 | 运行规则任务rule01, 立即执行 | 1) 任务运行成功2) 规则任务rule01详情页中, 分区配置变更成功 |
| 7 | 进入【校验结果查询】, 检查实例详情页面 | 实例停止运行, 实例状态依次变更: 【停止中】、【中途停止】 |
| 8 | 进入【规则任务管理】, 编辑rule01, 修改分区: 选择已有分区: factory_date=20260115/sale_date=20260201 , 点击数据预览 | 分区配置变更成功, 变更范围: |
| 9 | 后面配置不变, 保存后再次运行任务 | 表名悬浮提示中的分区配置信息 |
| 10 | 进入【校验结果查询】, 检查【运行中、等待运行】的rule01实例的详情页 | 规则详情页中, 表级报告中的表名和分区信息 后续生成的实例正常运行, 校验结果: 校验通过 |
| 11 | 检查后续生成的rule01实例的详情页 |  |
...
| 1 | 进入【数据资产】-【数据质量】-【规则任务管理】页面 | 进入成功 |
| 2 | 新建监控规则, 配置如下: | 配置成功 |
| 3 | 监控对象: - 规则名称: rule01 - 数据表: dwd_voyah_vehicle_sales_dates - 选择已有分区: factory_date=20260202 | 运行成功 |
| 4 | 监控规则: 完整性校验: - 生效范围: 字段级 - 字段: final_price - 统计函数: 字段取值校验 - 期望值: >= 0 | 校验结果: 校验不通过 |
| 5 | 调度属性: - 调度周期: 时 - 生效日期: T~T+1 - 间隔时间: 1小时 - 其它默认 - 实例生成方式: 「立即生成」 | 数据预览成功: 暂无数据 |
| 6 | 运行规则任务rule01, 立即执行 | 1) 任务运行成功2) 规则任务rule01详情页中, 分区配置变更成功 |
| 7 | 进入【校验结果查询】, 检查实例详情页面 | 实例停止运行, 实例状态依次变更: 【停止中】、【中途停止】 |
...
| 1 | 进入【数据资产】-【数据质量】-【规则任务管理】页面 | 进入成功 |
| 2 | 新建监控规则, 配置如下: | 配置成功 |
| 3 | 监控对象: - 规则名称: rule01 - 数据表: dwd_voyah_vehicle_sales_dates - 选择动态分区:  factory_date = bdp.system.bizdate | 运行成功 |
| 4 | 监控规则: 完整性校验: - 生效范围: 字段级 - 字段: final_price - 统计函数: 字段取值校验 - 期望值: >= 0 | 校验结果: 校验不通过 |
| 5 | 调度属性: - 调度周期: 时 - 生效日期: T~T+1 - 间隔时间: 1小时 - 其它默认 - 实例生成方式: 「立即生成」 | 数据预览成功: 3条数据 |
| 6 | 运行规则任务rule01, 立即执行 | 1) 任务运行成功2) 规则任务rule01详情页中, 分区配置变更成功 |
| 7 | 进入【校验结果查询】, 检查实例详情页面 | 实例停止运行, 实例状态依次变更: 【停止中】、【中途停止】 |
| 8 | 进入【规则任务管理】, 编辑rule01, 修改分区: 手动输入分区: factory_date=20260115 , 点击数据预览 | 分区配置变更成功, 变更范围: |
| 9 | 后面配置不变, 保存后再次运行任务 | 表名悬浮提示中的分区配置信息 |
| 10 | 进入【校验结果查询】, 检查【运行中、等待运行】的rule01实例的详情页 | 规则详情页中, 表级报告中的表名和分区信息 |
| 11 | 检查后续生成的rule01实例的详情页 | 后续生成的实例正常运行, 校验结果: 校验通过 |
| 12 | 检查后续rule01实例的校验结果 |  |
...
| 4 | 监控规则: 完整性校验: - 生效范围: 字段级 - 字段: final_price - 统计函数: 字段取值校验 - 期望值: >= 0 | 校验结果: 校验不通过 |
| 5 | 调度属性: - 调度周期: 时 - 生效日期: T~T+1 - 间隔时间: 1小时 - 其它默认 - 实例生成方式: 「立即生成」 | 数据预览成功: 3条数据 |
| 6 | 运行规则任务rule01, 立即执行 | 1) 任务运行成功2) 规则任务rule01详情页中, 分区配置不变 |
| 7 | 进入【校验结果查询】, 检查实例详情页面 | 分区配置不变, 实例继续运行, 结果不变: 校验不通过 |
| 8 | 进入【规则任务管理】, 编辑rule01, 修改分区: 手动输入分区: factory_date=20260202 , 点击数据预览 | 分区配置不变, 实例继续运行, 结果依旧不变: 校验不通过 |
| 9 | 后面配置不变, 保存后再次运行任务 |  |
| 10 | 进入【校验结果查询】, 检查【运行中、等待运行】的rule01实例的详情页 |  |
| 11 | 检查后续生成的rule01实例的详情页 |  |
