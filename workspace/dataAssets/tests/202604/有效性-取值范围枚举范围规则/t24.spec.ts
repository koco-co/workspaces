...
test.describe("【内置规则丰富】有效性，支持设置字段多规则的且或关系(#15695) - 校验结果查询", () => {
  test("验证取值范围&枚举范围规则校验「通过时不记录」明细数据且操作列不显示查看详情", async ({ page, step }) => {
    ...
    await step("步骤2: 找到 task_15695_or 最新实例记录，点击【查看详情】打开实例详情 → 实例详情页面打开，取值范围&枚举范围规则行数据加载完成", async () => {
    ...
    await step("步骤3: 查看规则行的质检结果列、详情说明列和操作列 → 质检结果为「校验通过」，详情说明含规则描述，操作列显示 --", async () => {
      const ruleRow = page.locator(".ant-table-row").filter({ hasText: "取值范围&枚举范围" }).first();
