...
test.describe("【内置规则丰富】有效性，支持设置字段多规则的且或关系(#15695) - 校验结果查询", () => {
  test("验证取值范围&枚举范围规则校验「不通过时下载」明细数据中校验字段标红展示", async ({ page, step }) => {
    ...
    // 步骤1：进入校验结果查询页面
    await step("步骤1: 进入【数据质量 → 校验结果查询】页面 → 校验结果查询页面打开，列表显示已有任务记录", async () => {
    ...
    await step("步骤2: 找到 task_15695_and 最新实例记录，打开实例详情，点击取值范围&枚举范围规则行操作列【查看详情】 → 明细数据页面打开，数据列表显示不通过记录共 4 条", async () => {
      const taskRow = page.locator(".ant-table-row").filter({ hasText: "task_15695_and" }).first();
      ...
      const ruleRow = page.locator(".ant-table-row").filter({ hasText: "取值范围&枚举范围" }).first();
    ...
    await step("步骤4: 查看明细数据页面中字段展示情况 → score 字段标红，不通过记录共 4 条", async () => {
      // 验证明细表格有数据行（4条不通过记录）
      const detailRows = page.locator(".ant-table-row");
      ...
      const redCell = page.locator("table tbody td").filter({ hasText: /score/ }).first()
        .or(page.locator("table tbody td[class*='red'], table tbody td[style*='red']").first());
      // 至少有一个被标红的单元格
      const redCellCount = await page.locator("table tbody td[class*='red'], table tbody td[style*='color: red'], table tbody td[style*='background']").count();
      // 验证表格中有数据行展示
