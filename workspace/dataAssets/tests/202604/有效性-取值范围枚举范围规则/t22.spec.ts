...
test.describe(
  "【内置规则丰富】有效性，支持设置字段多规则的且或关系(#15695) - 校验结果查询",
  () => {
    // 前置：
    ...
    test(
      "验证取值范围&枚举范围规则校验「不通过时可查看」明细且校验字段标红展示",
      async ({ page, step }) => {
        ...
        await step(
          ...
          async () => {
            ...
            const hasRedCell = await redScoreCell
            ...
            if (!hasRedCell) {
              ...
              await expect(
                headers.filter({ hasText: "score" }),
              ).toBeVisible({ timeout: 3000 });
              await expect(
                headers.filter({ hasText: "category" }),
              ).toBeVisible({ timeout: 3000 });
            } else {
