  ...
  test("验证在规则集中已保存的且关系规则编辑切换为或关系后保存成功", async ({ page, step }) => {
    ...
    await step(
      ...
      async () => {
        ...
        await expect(successMsg.filter({ hasText: /成功/ }).first()).toBeVisible({ timeout: 8000 });
        await expect(getRuleSetListRow(page, "ruleset_15695_and")).toBeVisible({ timeout: 10000 });
