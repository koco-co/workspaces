...
test.describe("【内置规则丰富】有效性，支持设置字段多规则的且或关系(#15695) - 规则集管理", () => {
  test("验证枚举值设置已选择in但未输入枚举值时点击保存提示校验错误", async ({ page, step }) => {
    ...
    await step("步骤3: 填写字段score，取值范围> 1，枚举值仅选择in不输入枚举值，关系选且，强规则 → 枚举值操作符显示in，枚举值输入区为空", async () => {
      // 选择字段：score
      ...
      const enumOpDisplay = page.locator(".ant-select-selection-item").filter({ hasText: "in" }).first();
      await expect(enumOpDisplay).toBeVisible({ timeout: 5000 });
    }, page.locator(".ant-form-item").filter({ hasText: /枚举值/ }).first());

    await step("步骤4: 点击保存按钮 → 保存失败，枚举值输入区展示红色校验错误提示「请输入枚举值」", async () => {
      await page.getByRole("button", { name: "保存" }).first().click();
      await page.waitForTimeout(1000);

      const errorMsg = page.locator(".ant-form-item-explain-error, .has-error .ant-form-explain, [class*='error']").filter({ hasText: /枚举值/ }).first();
      await expect(
        errorMsg.or(page.getByText("请输入枚举值"))
