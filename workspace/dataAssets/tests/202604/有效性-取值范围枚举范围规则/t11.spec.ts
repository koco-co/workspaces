...
test.describe("【内置规则丰富】有效性，支持设置字段多规则的且或关系(#15695) - 规则集管理", () => {
  test("验证取值范围和枚举值均未填写时点击保存提示至少填写一项", async ({ page, step }) => {
    ...
    await step("步骤3: 填写字段score，取值范围和枚举值均不填写，保持为空 → 字段已选score，取值范围和枚举值均为空", async () => {
      ...
      await expect(rangeRow).toBeVisible({ timeout: 5000 });
    }, page.locator(".ant-form-item").filter({ hasText: /取值范围/ }).first());

    await step("步骤4: 点击保存按钮 → 保存失败，展示红色校验错误提示「取值范围和枚举值至少填写一项」", async () => {
      await page.getByRole("button", { name: "保存" }).first().click();
      await page.waitForTimeout(1000);

      const errorMsg = page.locator(".ant-form-item-explain-error, .has-error .ant-form-explain, [class*='error']").filter({ hasText: /至少填写一项/ }).first();
      await expect(
        errorMsg.or(page.getByText("取值范围和枚举值至少填写一项"))
