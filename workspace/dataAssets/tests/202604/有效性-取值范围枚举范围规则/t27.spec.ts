...
test.describe("【内置规则丰富】有效性，支持设置字段多规则的且或关系(#15695) - 数据质量报告", () => {
  test("验证仅配置枚举值not in校验不通过时质量报告详情说明展示not in规则描述", async ({
    ...
    await step(
      ...
      async () => {
        await ensureQualityReportsReady(page, ["task_15695_enum_notin_fail"]);
    ...
    await step(
      ...
      async () => {
        await openQualityReportDetail(page, "task_15695_enum_notin_fail");
        const ruleRow = getQualityReportRuleRow(page, "枚举值");
        await expect(ruleRow).toBeVisible({ timeout: 10000 });
        await expect(ruleRow).toContainText(/枚举值not in\s*'4,5'/, { timeout: 5000 });
        await expect(ruleRow).toContainText(/约定范围外的值(?:的)?数量总计/, { timeout: 5000 });
