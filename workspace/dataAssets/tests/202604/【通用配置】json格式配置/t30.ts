// META: {"id":"t30","priority":"P1","title":"【P1】验证导入功能正常(重复则跳过, 1~5层key存在相同 -> 报错)"}
...
test.describe("【通用配置】json格式配置 - 通用配置-json格式校验管理", () => {
  test("【P1】验证导入功能正常(重复则跳过, 1~5层key存在相同 -> 报错)", async ({ page, step }) => {
    const dupKey = uniqueName("dupKey");
    const xlsxPath = path.join("/tmp", `t30_${Date.now()}.xlsx`);
    ...
    try {
      ...
      // 步骤2：创建含同层级重复 key 的 xlsx 文件（一层 Sheet，两行相同 dupKey）
      await step(
        "步骤2: 创建含同层级重复key的xlsx文件 → 文件创建成功",
        async () => {
      ...
      // 步骤3：点击【导入】，保持默认「重复则跳过」规则，上传文件并确定
      await importXlsx(page, step, xlsxPath, "重复则跳过");

      // 步骤4：断言导入失败，错误提示含重复 key 相关文案
      const errorHint = page
        .locator(".ant-modal:visible, .ant-message-notice, .ant-notification-notice, .ant-alert")
        .filter({ hasText: /同一个层级下的key名不可重复|相同/ });
      ...
      await step(
        ...
        async () => {
          const modalError = page
            ...
            .filter({ hasText: /同一个层级下的key名不可重复|层级.*重复|不可重复/ });
