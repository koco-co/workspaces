...
const CASE_TITLE = '【P1】验证下载明细数据中校验字段标红展示';
...
async function runKeyRangeCaseByDatasource(
  ...
  const packageName = uniqueName('tt31_' + (datasourceLabel.includes("Spark") ? "spark" : "doris"));
  ...
  await step('步骤4: 配置字段/校验方法/校验内容（' + datasourceLabel + '）', async () => {
    await configureKeyRangeRule(page, ruleForm, {
      ...
      keyNames: ["key1"],
      ruleStrength: "强规则",
      description: '【P1】验证下载明细数据中校验字段标红展示-' + datasourceLabel,
    });
