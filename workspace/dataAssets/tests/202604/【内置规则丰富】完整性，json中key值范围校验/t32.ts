// META: {"id":"t32","priority":"P1","title":"【P1】验证校验通过时结果查询页不显示明细入口"}
...
const CASE_TITLE = '【P1】验证校验通过时结果查询页不显示明细入口';
...
async function runKeyRangeCaseByDatasource(
  ...
  const packageName = uniqueName('tt32_' + (datasourceLabel.includes("Spark") ? "spark" : "doris"));
  ...
  await step('步骤4: 配置字段/校验方法/校验内容（' + datasourceLabel + '）', async () => {
    await configureKeyRangeRule(page, ruleForm, {
      ...
      keyNames: ["key1"],
      ruleStrength: "强规则",
      description: '【P1】验证校验通过时结果查询页不显示明细入口-' + datasourceLabel,
    });
