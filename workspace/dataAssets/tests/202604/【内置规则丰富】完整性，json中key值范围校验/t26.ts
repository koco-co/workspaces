...
const CASE_TITLE = '【P1】验证删除已被规则引用的key后执行校验任务不受影响';
...
async function runKeyRangeCaseByDatasource(
  ...
  const packageName = uniqueName('tt26_' + (datasourceLabel.includes("Spark") ? "spark" : "doris"));
  ...
  await step('步骤4: 配置字段/校验方法/校验内容（' + datasourceLabel + '）', async () => {
    await configureKeyRangeRule(page, ruleForm, {
      ...
      keyNames: ["key1"],
      ruleStrength: "强规则",
      description: '【P1】验证删除已被规则引用的key后执行校验任务不受影响-' + datasourceLabel,
    });
