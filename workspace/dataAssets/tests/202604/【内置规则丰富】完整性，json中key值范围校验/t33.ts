// META: {"id":"t33","priority":"P2","title":"【P2】验证校验失败时支持查看日志"}
...
const CASE_TITLE = '【P2】验证校验失败时支持查看日志';
...
async function runKeyRangeCaseByDatasource(
  ...
  const packageName = uniqueName('tt33_' + (datasourceLabel.includes("Spark") ? "spark" : "doris"));
  ...
  await step('步骤4: 配置字段/校验方法/校验内容（' + datasourceLabel + '）', async () => {
    await configureKeyRangeRule(page, ruleForm, {
      ...
      keyNames: ["key1"],
      ruleStrength: "强规则",
      description: '【P2】验证校验失败时支持查看日志-' + datasourceLabel,
    });
