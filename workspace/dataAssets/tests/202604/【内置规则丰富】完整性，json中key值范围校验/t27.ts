...
const CASE_TITLE = '【P1】验证key范围校验规则结合抽样功能正确执行';
...
async function runKeyRangeCaseByDatasource(
  ...
  const packageName = uniqueName('tt27_' + (datasourceLabel.includes("Spark") ? "spark" : "doris"));
  ...
  await step('步骤4: 配置字段/校验方法/校验内容（' + datasourceLabel + '）', async () => {
    await configureKeyRangeRule(page, ruleForm, {
      ...
      keyNames: ["key1"],
      ruleStrength: "强规则",
      description: '【P1】验证key范围校验规则结合抽样功能正确执行-' + datasourceLabel,
    });
