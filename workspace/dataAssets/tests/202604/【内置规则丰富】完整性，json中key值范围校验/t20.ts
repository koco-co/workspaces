// META: {"id":"t20","priority":"P1","title":"【P1】验证SparkThrift2.x数据源的string字段支持key范围校验"}
...
const CASE_TITLE = '【P1】验证SparkThrift2.x数据源的string字段支持key范围校验';
...
async function runKeyRangeCaseByDatasource(
  ...
  const packageName = uniqueName('tt20_' + (datasourceLabel.includes("Spark") ? "spark" : "doris"));
  ...
  await step('步骤4: 配置字段/校验方法/校验内容（' + datasourceLabel + '）', async () => {
    await configureKeyRangeRule(page, ruleForm, {
      ...
      keyNames: ["key1"],
      ruleStrength: "强规则",
      description: '【P1】验证SparkThrift2.x数据源的string字段支持key范围校验-' + datasourceLabel,
    });
