// META: {"id":"t34","priority":"P0","title":"【P0】验证质量报告中校验通过行的各列展示内容正确"}
...
const CASE_TITLE = '【P0】验证质量报告中校验通过行的各列展示内容正确';
...
async function runKeyRangeCaseByDatasource(
  ...
  const packageName = uniqueName('tt34_' + (datasourceLabel.includes("Spark") ? "spark" : "doris"));
  ...
  await step('步骤4: 配置字段/校验方法/校验内容（' + datasourceLabel + '）', async () => {
    await configureKeyRangeRule(page, ruleForm, {
      ...
      keyNames: ["key1"],
      ruleStrength: "强规则",
      description: '【P0】验证质量报告中校验通过行的各列展示内容正确-' + datasourceLabel,
    });
