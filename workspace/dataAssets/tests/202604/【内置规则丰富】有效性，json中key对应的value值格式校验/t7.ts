...
const CASE_TITLE = '【P1】验证校验key数据量超过200条时默认加载前200条展示';
...
async function runJsonFormatCaseByDatasource(
  ...
  const packageName = uniqueName('tt7_' + (datasourceLabel.includes("Spark") ? "spark" : "doris"));
  ...
  await step('步骤4: 配置格式-json格式校验规则（' + datasourceLabel + '）', async () => {
    await configureJsonFormatRule(page, ruleForm, {
      ...
      keyNames: ["key1"],
      ruleStrength: "强规则",
      description: '【P1】验证校验key数据量超过200条时默认加载前200条展示-' + datasourceLabel,
    });
