...
const CASE_TITLE = '【P1】验证选择非json或string类型字段时「格式-json格式校验」统计规则选项不可选';
...
async function runJsonFormatCaseByDatasource(
  ...
  const packageName = uniqueName('tt11_' + (datasourceLabel.includes("Spark") ? "spark" : "doris"));
  ...
  await step('步骤4: 配置格式-json格式校验规则（' + datasourceLabel + '）', async () => {
    await configureJsonFormatRule(page, ruleForm, {
      ...
      keyNames: ["key1"],
      ruleStrength: "强规则",
      description: '【P1】验证选择非json或string类型字段时「格式-json格式校验」统计规则选项不可选-' + datasourceLabel,
    });
