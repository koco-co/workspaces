...
const CASE_TITLE = '【P1】验证配置格式-json格式校验规则时结合抽样功能执行校验结果正确';
...
async function runJsonFormatCaseByDatasource(
  ...
  const packageName = uniqueName('tt22_' + (datasourceLabel.includes("Spark") ? "spark" : "doris"));
  ...
  await step('步骤4: 配置格式-json格式校验规则（' + datasourceLabel + '）', async () => {
    await configureJsonFormatRule(page, ruleForm, {
      ...
      keyNames: ["key1"],
      ruleStrength: "强规则",
      description: '【P1】验证配置格式-json格式校验规则时结合抽样功能执行校验结果正确-' + datasourceLabel,
    });
