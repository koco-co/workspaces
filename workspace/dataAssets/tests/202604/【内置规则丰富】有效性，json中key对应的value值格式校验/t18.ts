// META: {"id":"t18","priority":"P1","title":"【P1】验证「格式-json格式校验」规则在Hive 2.x数据源下执行正确"}
...
const CASE_TITLE = '【P1】验证「格式-json格式校验」规则在Hive 2.x数据源下执行正确';
...
async function runJsonFormatCaseByDatasource(
  ...
  await step('步骤4: 配置格式-json格式校验规则（' + datasourceLabel + '）', async () => {
    await configureJsonFormatRule(page, ruleForm, {
      ...
      keyNames: ["key1"],
      ruleStrength: "强规则",
      description: '【P1】验证「格式-json格式校验」规则在Hive 2.x数据源下执行正确-' + datasourceLabel,
    });
