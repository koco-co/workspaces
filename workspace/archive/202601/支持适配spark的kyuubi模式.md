---
suite_name: "支持适配spark的kyuubi模式"
description: "支持适配spark的kyuubi模式用例归档"
tags:
  - "联动"
  - "多表"
  - "单表"
...
| 编号 | 步骤 | 预期 |
| ---- | ---- | ---- |
| 1 | 1）提交离线任务 2）查看数据地图视图的血缘关系-表级血缘 | 表级血缘关系正确： |
| 2 | 1）提交离线任务 2）查看数据地图视图的血缘关系-字段级血缘 | 字段级血缘关系正确 |
...
| 编号 | 步骤 | 预期 |
| ---- | ---- | ---- |
| 1 | 1）点击【元数据】-【元数据同步】-【新增周期同步任务】 2）选择数据源${DATASOURCE_TYPE} 3）选择数据库${DATABASE} 4）选择数据表${TABLE} 5）依次点击【添加】_【下一步】_【新增】按钮 | 表同步成功， 且表详情信息展示正确， 数据预览正确 |
...
| 编号 | 步骤 | 预期 |
| ---- | ---- | ---- |
| 1 | 新增「元数据同步任务」，选择spark的数据源，选择库表，库表选择情况见前置条件，点击「临时运行」 | 新增一条元数据同步实例，运行中 |
...
##### 【P1】验证质量和离线绑定，多规则包-【表行数-弱规则-校验不通过】-校验结果正确
...
##### 【P1】验证质量和离线绑定，多规则包-【表行数-校验通过】-校验结果正确
...
##### 【P1】验证质量和离线绑定，单规则包-【表行数-弱规则-校验不通过】结果正确
...
##### 【P1】验证质量和离线绑定，单规则包-【表行数-校验通过】结果正确
...
| 2 | 右表选择前置中的表mysql_demo_data_types_source，点击下一步 | 1）字段类型一致的进行映射 2）不一致的不映射并提示：主表与对照表字段类型不一致，无法比对 |
| 3 | 同名映射 | 勾选成功 |
| 4 | 勾选user_id为主键 | 勾选成功 |
| 5 | 勾选【记录数百分比差异】-0% | 创建成功 |
| 6 | 创建规则 | 质量实例校验不通过 |
| 7 | 立即运行规则，查看结果 | 修改成功 |
| 8 | 编辑规则，修改【记录数百分比差异】-50% | 质量实例校验通过 |
| 9 | 立即运行规则，查看结果 |  |
...
| 编号 | 步骤 | 预期 |
| ---- | ---- | ---- |
| 1 | 新建单表规则，选择doris表 规则选择完整性校验-字段级，分别创建空值数、空值率、空串数、空串率的子规则，配置使校验通过 | 新建成功 |
| 2 | 立即运行查看结果 | 结果正确 |
| 3 | 修改规则，配置使其都校验不通过 | 编辑成功 |
| 4 | 立即运行查看结果 | 结果正确 |

##### 【P0】验证质量和离线绑定，多规则包-【表行数-强规则-校验不通过】-校验结果正确
...
| 编号 | 步骤 | 预期 |
| ---- | ---- | ---- |
| 1 | 进入「数据资产-数据质量」，新建单表校验，选择表doris2 规则：选择表行数，固定值>3,强规则。再任意添加其他两种类型的规则，使校验通过 调度：自动关联离线调度周期。规则包个数3 绑定任务：前置条件saprk2spark | 新建成功 |
| 2 | 等待离线周期实例运行，进入「数据资产-数据质量-任务实例查询」 | 1）页面生成一条实例，校验未通过 2）查看明细展示doris_demo_data_types_source中的三条数据 |
| 3 | 进入「离线开发-运维中心-周期任务实例」 | 离线任务运行失败 |
| 4 | 进入「离线开发-运维中心-周期任务」，对saprk2spark任务补当前数据及下游，查看结果 | 1）补数据质量工作流中有3个规则包，离线任务运行失败，报错质量任务校验不通过 2）质量生成一条实例，校验不通过 |

##### 【P0】验证质量和离线绑定，单规则包-【表行数-添加过滤条件】-校验结果正确
...
| 编号 | 步骤 | 预期 |
| ---- | ---- | ---- |
| 1 | 新建单表校验，选择表spark1 规则：选择表行数固定值>0，强规则 过滤条件： time >= '${utc0yyyy_MM_dd_HH_3H_00_00}' and time <  '${utc0yyyy_MM_dd_HH_00_00}' 调度：自动关联离线调度周期。规则包个数1 绑定任务：前置条件spark2spark | 新建成功 |
| 2 | 等待离线实例运行，查看质量实例 | 质量新增一条实例，状态为运行中 |
| 3 | 运行结束后，查看离线和质量实例的运行结果 | 1）质量实例：校验通过 2）离线实例：运行成功 |
| 4 | 进入离线，对spark2spark任务补数据-当前任务及下游，查看结果 | 同步骤2-3 |

##### 【P0】验证质量和离线绑定，单规则包-【表行数-强规则-校验不通过】结果正确
...
| 4 | 进入「离线开发-运维中心-周期任务」，对spark_2_spark任务补当前数据及下游，查看结果 | 1）补数据质量工作流中有1个规则包，离线任务运行失败，报错质量任务校验不通过 2）质量生成一条实例，校验不通过 |
...
| 编号 | 步骤 | 预期 |
| ---- | ---- | ---- |
| 1 | 点击「创建项目」按钮，配置如下 「项目名称」输入「超64字符文本」 「项目标识」输入「超64字符文本&特殊字符」 「管理员」不选择 「项目描述」输入「测试项目描述信息」 点击「确定」按钮 | 提示「项目名称」输入框64字符限制； 提示「项目标识」输入框64字符限制以及特殊字符不支持提示； 「管理员」必填项未填提示； |
| 2 | 点击「创建项目」按钮，配置如下 「项目名称」输入「测试项目」 「项目标识」输入「测试项目标识」 「管理员」选择${已引入到资产平台的用户,例如admin@dtstack.com} 「项目描述」输入「测试项目描述信息」 点击「确定」按钮 | 质量项目创建成功 |
