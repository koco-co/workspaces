---
suite_name: "【内置规则丰富】一致性，多表数据一致性校验"
description: "【内置规则丰富】一致性，多表数据一致性校验用例归档"
tags:
  - "正确"
  - "失败"
  - "告正确"
...
| 3 | 「监控规则」配置如下： 「监控规则」新增「一致性校验」 「校验类型」选择为「多表数据一致性比对」 「选择校验字段」选择为「user_name；age」 「选择校验表主键」选择为「id」 「选择对比表」为「${TABLE}」 「选择对比表主键」为「id」 「比对字段设置」配置为「校验表-user_name-->对比表-age；校验表-age-->对比表-user_name」 「强弱规则」选择「弱规则」 「规则描述」输入「测试规则」 不配置「对比细节设置」 | 监控规则配置完成； 进入「调度属性」页面 |
| 4 | 配置「调度属性」，保存规则，且「调度属性」中配置「规则报告」为最新结果 | 规则保存成功 |
| 5 | 临时运行规则，查看质量报告 | 页面包含： 1）报告进入目录 2）质量评估汇总区域 3）规则校验明细区域 |
| 6 | 查看「质量评估汇总」区域 | 显示为： 1）规则名：「test_rule」 2）数据源：「${DATASOURCE}」 3）数据库：「${DATABASE}」 4）检测数据范围：-- 5）表行数：「${表行数}」 6）抽样行数：-- 7）字段数：「${字段数}」 8）车辆数：「${车辆数}」 9）校验规则数：2 10）校验通过率：50% |
| 7 | 查看「规则校验明细」区域 | 显示为： 1）规则类型：一致性校验 2）规则名称：多表数据一致性校验 3）字段名称：user_name;age 4）字段类型：STRING;TINYINT 5）质验结果：校验未通过 6）未通过原因：多表数据一致性校验未通过 7）详情说明：对比表为「${TABLE}」，逻辑主键未匹配的共0条数据，逻辑主键匹配，但数据不匹配的共2条数据 8）最近一次校验结束时间：「${校验结束时间}」 9）操作：查看详情 |
| 8 | 点击「查看详情」按钮 | 显示不通过的明细数据 |

##### 【P0】验证「多表数据一致性比对」任务通过质量报告正确
...
| 4 | 配置「调度属性」，保存规则，且「调度属性」中配置「规则报告」为最新结果 | 规则保存成功 |
| 5 | 临时运行规则，查看质量报告 | 页面包含： 1）报告进入目录 2）质量评估汇总区域 3）规则校验明细区域 |
| 6 | 查看「质量评估汇总」区域 | 显示为： 1）规则名：「test_rule」 2）数据源：「${DATASOURCE}」 3）数据库：「${DATABASE}」 4）检测数据范围：-- 5）表行数：「${表行数}」 6）抽样行数：-- 7）字段数：「${字段数}」 8）车辆数：「${车辆数}」 9）校验规则数：1 10）校验通过率：100% |
| 7 | 查看「规则校验明细」区域 | 显示为： 1）规则类型：一致性校验 2）规则名称：多表数据一致性校验 3）字段名称：user_name;age 4）字段类型：STRING;TINYINT 5）质验结果：校验通过 6）未通过原因：-- 7）详情说明：对比表为「${TABLE}」，校验字段的数据一致性符合规则 8）最近一次校验结束时间：「${校验结束时间}」 9）操作：-- |
...
| 4 | 查看「比对规则」内容 | 可勾选： 1）数值差异百分比，对比表之间的数值型数据时，差距百分比小于等于xx%时候，计为成功匹配 2）数值差异绝对值，对比表之间的数值型数据时，差距绝对值小于等于xx时候，计为成功匹配 3）数值对比忽略小数点，忽略小数点后xx位 4）字符不区分大小写，对比左右表的字符串型数据时，不区分大小写 5）空值与NULL等价，对比左右表的数据时，认为空值与NULL值是相等的 |
...
| 1 | 进入「资产-数据质量-规则任务配置-监控对象」页面 | 页面正常打开 |
| 2 | 「规则名称」输入「test_rule」 「选择数据源」选择「${DATASOURCE}」 「选择数据库」选择「${DATABASE}」 「选择数据表」选择「${TABLE}」 | 监控对象配置成功； 进入「监控规则」配置页 |
| 3 | 「监控规则」配置如下： 「监控规则」新增「一致性校验」 「校验类型」选择为「多表数据一致性比对」 「选择校验字段」选择为「user_name；age」 「选择校验表主键」选择为「id」 「选择对比表1」为「${TABLE}」 「选择对比表1主键」为「id1」 「选择对比表2」为「${TABLE}」 「选择对比表2主键」为「id2」 「比对字段设置」配置为 「校验表-user_name-->对比表1-age，对比表2-user_name；校验表-age-->对比表1-user_name，对比表2-age」 「强弱规则」选择「弱规则」 「规则描述」输入「测试规则」 不配置「对比细节设置」 | 监控规则配置完成； 进入「调度属性」页面 |
| 4 | 配置「调度属性」，保存规则，且「调度属性」中配置「规则报告」为最新结果 | 规则保存成功 |
| 5 | 临时运行规则，查看实例详情及质量报告 | 实例运行结果不通过，且实例详情展示正确，质量报告展示正确 |
