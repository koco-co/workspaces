...
| ---- | -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| 1    | 进入【数据质量 → 通用配置】页面，等待json格式校验管理列表数据加载完成                                                                  | json格式校验管理页面正常打开，列表加载完成                                   |
| 2    | 点击【导入】按钮，选择任意重复处理规则（重复则跳过或重复则覆盖更新均可），上传包含同层级重复key的XLSX文件，点击【确定】按钮            | 导入失败，系统报错提示「同一个层级下的key名不可重复」，列表数据不变           |

##### 【P0】验证导入覆盖更新模式-多层级key新增与更新逻辑
...
2) json格式校验管理列表中已有如下数据：
   - 第1层key: existParent（value格式: ^[a-z]+$）
   - 第2层key: existChild（父级为 existParent，value格式为空）
3) 准备 XLSX 导入文件，内容如下：
   一层 Sheet:
   | *key         | 中文名称   | value格式    |
   | existParent  | 已有父键   | ^[A-Z]+$     |
   | newParent    | 新增父键   | ^\d+$        |

   二层 Sheet:
   | *上一层级的key名 | *key         | 中文名称   | value格式  |
   | existParent      | existChild   | 已有子键   | ^[0-9]+$   |
   | existParent      | newChild     | 新增子键   | ^[a-z]+$   |
   | missingParent    | orphanChild  | 孤儿子键   | ^test$     |
```
...
| 编号 | 步骤                                                                                                                                   | 预期                                                                                                                                                                                                                                                                                                                                                                    |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | 进入【数据质量 → 通用配置】页面，等待json格式校验管理列表数据加载完成                                                                  | 页面正常加载，existParent 的 value格式显示 ^[a-z]+$，展开后 existChild 的 value格式显示为空（-）                                                                                                                                                                                                                                                                       |
| 2    | 点击【导入】按钮，将重复处理规则切换为「重复则覆盖更新」，上传准备的XLSX文件，点击【确定】按钮                                          | 系统提示部分导入结果：<br>1) 1层 existParent 已存在 → 更新: value格式由 ^[a-z]+$ 更新为 ^[A-Z]+$，中文名称更新为「已有父键」<br>2) 1层 newParent 不存在 → 新增: 列表中出现 newParent，value格式显示 ^\d+$<br>3) 2层 existParent→existChild 已存在且value不存在 → 更新: existChild 的 value格式更新为 ^[0-9]+$<br>4) 2层 existParent→newChild 不存在 → 新增: existParent 展开后显示 newChild<br>5) 2层 missingParent→orphanChild → 报错: 上一层级key名 missingParent 不存在 |

##### 【P0】验证导入跳过模式-多层级key跳过与新增逻辑
...
1) 使用 admin 账号登录系统
2) json格式校验管理列表中已有如下数据：
   - 第1层key: skipParent（value格式: ^[a-z]+$，中文名称: 原始父键）
   - 第2层key: skipChild（父级为 skipParent，value格式为空）
3) 准备 XLSX 导入文件，内容如下：
   一层 Sheet:
   | *key         | 中文名称   | value格式    |
   | skipParent   | 修改父键   | ^[A-Z]+$     |
   ...
   二层 Sheet:
   | *上一层级的key名 | *key         | 中文名称   | value格式  |
   | skipParent       | skipChild    | 修改子键   | ^[0-9]+$   |
   | skipParent       | addChild     | 新增子键   | ^[a-z]+$   |
   | noParent         | lostChild    | 丢失子键   | ^test$     |
```
...
| 编号 | 步骤                                                                                                                                   | 预期                                                                                                                                                                                                                                                                                                                                                                    |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | 进入【数据质量 → 通用配置】页面，等待json格式校验管理列表数据加载完成                                                                  | 页面正常加载，skipParent 的 value格式显示 ^[a-z]+$，中文名称显示「原始父键」                                                                                                                                                                                                                                                                                           |
| 2    | 点击【导入】按钮，确认重复处理规则为「重复则跳过」，上传准备的XLSX文件，点击【确定】按钮                                                | 系统提示部分导入结果：<br>1) 1层 skipParent 已存在 → 跳过: value格式仍为 ^[a-z]+$，中文名称仍为「原始父键」，不变<br>2) 1层 brandNew 不存在 → 新增: 列表中出现 brandNew，value格式显示 ^\d+$<br>3) 2层 skipParent→skipChild 已存在且value不存在 → 跳过: skipChild 的 value格式仍为空（-），不变<br>4) 2层 skipParent→addChild 不存在 → 新增: skipParent 展开后显示 addChild<br>5) 2层 noParent→lostChild → 报错: 上一层级key名 noParent 不存在 |
...
| 2    | 点击列表右上角【导出】按钮（或下载按钮），等待文件下载完成                                                                | 文件下载成功，打开下载的xlsx文件，核对导出的key记录总数与列表中显示的总记录数一致，验证是否存在下载数量上限限制                         |
