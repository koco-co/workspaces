...
2、数据资产-平台管理-数据源管理中已存在Doris3.x数据源
3、Doris3.x数据源已授权给质量项目
4、Doris3.x数据源已在「元数据同步」模块中自动同步或手动同步5条数据表, 建表语句如下:
...
```
1、使用 admin 账号登录系统
2、Doris 3.x 数据源已引入资产平台，元数据同步完成
3、存在已同步的 Doris 3.x 数据表 vehicle_info_part1，该表尚未被当前用户订阅
```
...
| 编号 | 步骤                                           | 预期                                                                                                                                                                |
| ---- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | 进入【数据资产】-【元数据】-【数据地图】页面   | 成功进入【数据地图】页面，页面内容正常加载，无报错                                                                                                                  |
...
```
1、使用账号 admin / 密码 Admin@123 登录系统
2、已订阅 Doris 3.x 数据表 vehicle_info_part1
...
| 编号 | 步骤                                                                                 | 预期                                                                          |
| ---- | ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------- |
| 1    | 进入【数据资产】-【元数据】-【订阅的数据】页面                                       | 成功进入【订阅的数据】页面，订阅的数据表记录列表正常加载，无报错              |
...
```
1、使用账号 admin / 密码 Admin@123 登录系统
2、已订阅 Doris 3.x 数据表 vehicle_info_part1，提醒方式已配置
3、离线开发中已对接 Doris 3.x 集群
...
1、使用账号 admin / 密码 Admin@123 登录系统
2、Doris 3.x 数据源已引入资产平台，元数据同步完成
3、Doris 3.x 数据库中存在多张表，部分表已通过离线任务建立血缘关系（如通过INSERT INTO/CREATE TABLE AS SELECT等SQL），部分表无任何上下游血缘关系
...
| 编号 | 步骤                                                                              | 预期                                                                                                                                                                   |
| ---- | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | 进入【数据资产】-【元数据】-【血缘分析】页面                                      | 成功进入【血缘分析】页面，已添加监控的数据库列表正常加载，无报错                                                                                                       |
...
```
1、使用账号 admin / 密码 Admin@123 登录系统
2、资产平台已自动对接离线平台中带Meta标识的Doris 3.x数据源
3、在离线开发平台中创建DorisSQL任务并执行, 建表语句如下:
...
| 3    | 1) 配置内容如下: 规则名称: test_rule 自动同步过滤维度: 数据表表名 过滤内容: vehicle_info_part\* 数据源: 测试数据源\_Doris 数据库: doris_quality_project 2) 配置完毕后,点击确定 | 新增过滤规则成功                                                                                                       |
| 4    | 1) 点击「周期同步」页签, 新增周期同步任务, 配置如下: 数据源: 测试数据源\_Doris 数据库: doris_quality_project 数据表: vehicle_info_part6~10 2) 配置完毕后, 点击「临时同步」     | 1) 「新增周期同步任务」弹窗关闭, 并Toast提示: 临时同步成功 2) 同步状态为同步中, 等待一段时间后, 同步状态变更为同步完成 |
...
| 3    | 「数据源」: 测试数据源\_Doris 「数据库」: doris_quality_db 「数据表」: vehicle_info_part1 ~3 | 配置成功                                                                                                               |
...
| 1    | 进入【数据资产】-【元数据】-【元数据同步】页面，等待页面加载完成                             | 成功进入【元数据同步】页面，同步任务列表正常加载，无报错                                                                                                              |
| 2    | 点击「新增周期同步任务」                                                                     | 弹出「新增周期同步任务」弹窗                                                                                                                                          |
| 3    | 「数据源」: 测试数据源\_Doris 「数据库」: doris_quality_db 「数据表」: vehicle_info_part4 ~5 | 配置成功                                                                                                                                                              |
...
```
1、使用账号 admin / 密码 Admin@123 登录系统
2、Doris 3.x 数据源已引入资产平台
3、已在 Doris 3.x 中创建测试表:
...
| 编号 | 步骤                                                                     | 预期                                                                                                             |
| ---- | ------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| 1    | 进入【数据资产】-【元数据】-【元数据同步】页面                           | 成功进入【元数据同步】页面，同步任务列表正常加载，无报错                                                         |
...
1、使用账号 admin / 密码 Admin@123 登录系统
2、Doris 3.x 数据源已引入资产平台
3、已在 Doris 3.x 中创建包含特有字段类型的测试表:
...
| 编号 | 步骤                                                         | 预期                                                                                                                         |
| ---- | ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| 1    | 进入【数据资产】-【元数据】-【元模型管理】页面               | 成功进入【元模型管理】页面，元模型列表正常加载，无报错                                                                       |
| 2    | 下滑找到Doris 3.x元模型, 点击「编辑元模型」                  | 1) 展示的技术属性与Doris 2.x保持一致 2) 支持维护通用业务属性、个性业务属性 3) 通用业务属性默认存在两个字段: 表中文名和负责人 |
| 3    | 进入【数据资产】-【元数据】-【元数据管理】页面               | 成功进入【元数据管理】页面，数据源列表正常加载，无报错                                                                       |
| 4    | 选择数据源类型为「Doris 3.x」的数据源, 点击进入              | 成功进入 Doris 3.x 数据源详情，数据库列表正常加载，无报错                                                                    |
...
| 编号 | 步骤                                                                                                                                      | 预期                                                                                                                         |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| 1    | 进入【数据资产】-【元数据】-【元数据管理】页面                                                                                            | 成功进入【元数据管理】页面，数据源列表正常加载，无报错                                                                       |
| 2    | 选择数据源类型为「Doris 3.x」的数据源, 点击进入                                                                                           | 成功进入 Doris 3.x 数据源详情，数据库列表正常加载，无报错                                                                    |
| 3    | 点击数据库, 进入数据表页面, 选择 vehicle_info_part1 记录, 点击「删除」按钮                                                                | 1) 弹出二次确认弹窗: 「确定删除表 vehicle_info_part1 吗?」 2) 删除方式支持删除元数据表和删除源表 3) 输入的表名一致后可删除表 |
| 4    | 「删除方式」选择「删除元数据表」, 输入 vehicle_info_part1 后, 点击「确定」                                                                | 1) 弹窗关闭, 该表信息从资产平台及其他子产品中删除完成 2) 可以通过元数据同步中, 重新将该表同步至资产平台                      |
| 5    | 进入「离线开发-数据开发-周期任务」, 创建DorisSQL任务, 执行查询语句: SELECT \* FROM vehicle_info_part1;                                    | SQL任务运行成功, 源表仍存在                                                                                                  |
| 6    | 返回「资产平台-元数据管理」中, 再次选择 vehicle_info_part2 记录, 点击删除按钮, 选择「删除源表」, 输入 vehicle_info_part2 后, 点击「确定」 | 弹窗关闭, 该表信息从源库中删除                                                                                               |
| 7    | 进入「离线开发-数据开发-周期任务」, 创建DorisSQL任务, 执行查询语句: SELECT \* FROM vehicle_info_part2;                                    | SQL任务运行失败, 源表不存在                                                                                                  |
...
| 5    | 进入「离线开发-数据开发-周期任务」, 创建DorisSQL任务, 执行查询语句: USE doris_quality_db; | SQL任务运行成功, 源库仍存在                                                                   |
...
1、使用账号 admin / 密码 Admin@123 登录系统
2、Doris 3.x 数据源已引入资产平台，元数据同步完成
3、存在已同步的 Doris 3.x 数据表 vehicle_info_part1，且已配置过业务属性
...
```
1、使用账号 admin / 密码 Admin@123 登录系统
2、Doris 3.x 数据源已引入资产平台并授权给质量项目
3、已配置并执行了至少一条 Doris 3.x 数据源的规则任务，且存在校验通过和校验失败的实例记录
...
| 编号 | 步骤                                       | 预期                                                                                                                                |
| ---- | ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- |
| 1    | 进入【数据资产】-【数据质量】-【概览】页面 | 成功进入【概览】页面，统计数据卡片正常加载，无报错                                                                                  |
...
| 编号 | 步骤                                                                                                                                                                            | 预期                                                                                                 |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| 1    | 进入【数据资产】-【数据质量】-【规则任务配置】页面                                                                                                                              | 成功进入【规则任务配置】页面，规则任务列表正常加载，无报错                                           |
| 2    | 点击「新建监控规则」按钮，进入监控规则配置页面                                                                                                                                  | 成功进入监控规则配置页面，规则配置表单正常加载，无报错                                               |
...
```
1、使用账号 admin / 密码 Admin@123 登录系统
2、已在 Doris 3.x 数据源中创建测试表并写入测试数据
3、已进入【数据资产】-【数据质量】-【规则任务配置】页面
```
...
| 编号 | 步骤                                                                                                               | 预期                                                                                         |
| ---- | ------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------- |
| 1    | 新建单表规则，选择doris表 规则选择准确性校验，分别创建求和、求平均、负值比、零值比、正值比的子规则，配置使校验通过 | 新建成功，规则任务列表中新增一条记录                                                         |
| 2    | 点击「立即运行」按钮，等待运行结束后查看任务实例结果                                                               | 任务实例状态为「校验通过」，各子规则的实际值与期望值匹配                                     |
| 3    | 编辑该规则，修改期望值配置使各子规则校验不通过，点击「保存」                                                       | 编辑成功，规则配置信息更新                                                                   |
| 4    | 再次点击「立即运行」按钮，等待运行结束后查看任务实例结果                                                           | 任务实例状态为「校验未通过」，各子规则的实际值与期望值不匹配，详情中可查看具体不通过的子规则 |
...
```
1、使用账号 admin / 密码 Admin@123 登录系统
2、已在 Doris 3.x 数据源中创建测试表并写入包含各类规范性场景的测试数据
3、已进入【数据资产】-【数据质量】-【规则任务配置】页面
```
...
| 编号 | 步骤                                                                                                                                                                                                 | 预期                                                                                         |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| 1    | 新建单表规则，选择doris表 规则选择规范性校验，分别创建取值范围、枚举范围、枚举个数、身份证号、手机号、邮箱、最大长度、最小长度、字符串长度、数据精度、空值数、重复数、枚举值的子规则，配置使校验通过 | 新建成功，规则任务列表中新增一条记录                                                         |
| 2    | 点击「立即运行」按钮，等待运行结束后查看任务实例结果                                                                                                                                                 | 任务实例状态为「校验通过」，各子规则的实际值与期望值匹配                                     |
| 3    | 编辑该规则，修改期望值配置使各子规则校验不通过，点击「保存」                                                                                                                                         | 编辑成功，规则配置信息更新                                                                   |
| 4    | 再次点击「立即运行」按钮，等待运行结束后查看任务实例结果                                                                                                                                             | 任务实例状态为「校验未通过」，各子规则的实际值与期望值不匹配，详情中可查看具体不通过的子规则 |
...
```
1、使用账号 admin / 密码 Admin@123 登录系统
2、已在 Doris 3.x 数据源中创建测试表并写入包含重复数据的测试数据
3、已进入【数据资产】-【数据质量】-【规则任务配置】页面
```
...
| 编号 | 步骤                                                                                                                 | 预期                                                                                         |
| ---- | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| 1    | 新建单表规则，选择doris表 规则选择唯一性校验，分别创建重复数、重复率、非重复个数、非重复占比的子规则，配置使校验通过 | 新建成功，规则任务列表中新增一条记录                                                         |
| 2    | 点击「立即运行」按钮，等待运行结束后查看任务实例结果                                                                 | 任务实例状态为「校验通过」，各子规则的实际值与期望值匹配                                     |
| 3    | 编辑该规则，修改期望值配置使各子规则校验不通过，点击「保存」                                                         | 编辑成功，规则配置信息更新                                                                   |
| 4    | 再次点击「立即运行」按钮，等待运行结束后查看任务实例结果                                                             | 任务实例状态为「校验未通过」，各子规则的实际值与期望值不匹配，详情中可查看具体不通过的子规则 |
...
| 编号 | 步骤                                                                                                                                     | 预期                                                     |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| 1    | 新建单表规则，选择doris表 规则选择完整性校验-字段级，涵盖字段级下的所有子规则类型，配置使校验通过 调度时间为17分钟后，调度方式：立即生成 | 新建成功，规则任务列表中新增一条记录                     |
| 2    | 17分钟后，进入【质量实例】页面查看该任务的实例记录                                                                                       | 生成一条实例，状态为「运行中」                           |
| 3    | 等待运行结束后查看任务实例详情                                                                                                           | 任务实例状态为「校验通过」，各子规则的实际值与期望值匹配 |
| 4    | 第二天进入【质量实例】页面查看该任务                                                                                                     | 周期时间自动生成一条新实例，运行后状态为「校验通过」     |
...
| 编号 | 步骤                                                                                                                                | 预期                                                     |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| 1    | 新建单表规则，选择doris表 规则选择完整性校验-字段级，涵盖字段级下的所有子规则类型，配置使校验通过 调度时间为17分钟后，调度方式：T+1 | 新建成功，规则任务列表中新增一条记录                     |
| 2    | 17分钟后，进入【质量实例】页面查看该任务                                                                                            | 该任务无实例生成（T+1模式当天不触发）                    |
| 3    | 第二天进入【质量实例】页面查看该任务                                                                                                | 周期时间自动生成一条实例，状态为「运行中」或已完成       |
| 4    | 等待运行结束后查看任务实例详情                                                                                                      | 任务实例状态为「校验通过」，各子规则的实际值与期望值匹配 |
...
5、select 1 from doris_demo_data_types_source;
...
3、离线已存在doris2doris数据同步任务：doris1-->doris2 覆盖

4、doris2为空表，doris1中有3条表数据，覆盖模式
...
```
1、使用 admin 账号登录系统
2、数据质量规则任务已运行过，存在与 Doris 3.x 数据源相关的任务实例记录，包含：
   - 表名包含「vehicle_info」的规则任务实例
   - 表名包含「vehicle_info_part」的规则任务实例，最近修改人为 admin@dtstack.com
...
| 编号 | 步骤                                                                                                                               | 预期                                                                         |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| 1    | 进入【数据资产】-【数据质量】-【任务实例查询】页面，等待页面加载完成                                                               | 成功进入【任务实例查询】页面，任务实例列表正常加载，无报错                   |
| 2    | 在页面顶部搜索框中输入表名/任务名称 vehicle_info，点击【查询】按钮，等待列表刷新                                                   | 查询出「表」的表名中所有包含 vehicle_info 的规则记录                         |
| 3    | 清空所有查询条件，在搜索框中输入不存在的表名 nonexistent_table_xyz，点击【查询】按钮，等待列表刷新                                 | 显示「暂无数据」                                                             |
| 4    | 清空所有查询条件，在搜索框中输入表名 vehicle_info_part，在「修改人」下拉框中选择 admin@dtstack.com，点击【查询】按钮，等待列表刷新 | 查询出表名包含 vehicle_info_part 且最近修改人为 admin@dtstack.com 的规则记录 |
| 5    | 清空所有查询条件，勾选「我收藏的表」复选框，点击【查询】按钮，等待列表刷新                                                         | 查询出操作中仅为「取消收藏」的表                                             |
| 6    | 清空所有查询条件，在分页组件中选择「10条/页」，等待列表刷新                                                                        | 当前页面规则数量变更为10条                                                   |
| 7    | 点击分页组件中的指定页码数字                                                                                                       | 跳转至对应的页码页面，页面显示对应页的记录                                   |
| 8    | 点击分页组件中的「<」按钮                                                                                                          | 向前翻页，显示上一页内容                                                     |
| 9    | 点击分页组件中的「>」按钮                                                                                                          | 向后翻页，显示下一页内容                                                     |
| 10   | 在分页组件中切换每页展示数量                                                                                                       | 每页展示记录数变更为切换后的数量                                             |
...
```
1、使用 admin 账号登录系统
2、数据质量规则任务已运行过，已生成质量报告记录，包含：
   - 报告名称包含「vehicle_info」的报告记录
   - 表名包含「vehicle_info_part」、最近修改人为 admin@dtstack.com 的报告记录
...
| 编号 | 步骤                                                                                                                                   | 预期                                                                         |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| 1    | 进入【数据资产】-【数据质量】-【质量报告】页面，等待页面加载完成                                                                       | 成功进入【质量报告】页面，报告列表正常加载，无报错                           |
| 2    | 在页面顶部「报告名称」输入框中输入 vehicle_info，点击【查询】按钮，等待列表刷新                                                        | 查询出报告名称中所有包含 vehicle_info 的记录                                 |
| 3    | 清空所有查询条件，在「表名」输入框中输入 vehicle_info_part，在「修改人」下拉框中选择 admin@dtstack.com，点击【查询】按钮，等待列表刷新 | 查询出表名包含 vehicle_info_part 且最近修改人为 admin@dtstack.com 的规则记录 |
| 4    | 清空所有查询条件，在分页组件中选择「10条/页」，等待列表刷新                                                                            | 当前页面规则数量变更为10条                                                   |
| 5    | 点击分页组件中的指定页码数字                                                                                                           | 跳转至对应的页码页面，页面显示对应页的记录                                   |
| 6    | 点击分页组件中的「<」按钮                                                                                                              | 向前翻页，显示上一页内容                                                     |
| 7    | 点击分页组件中的「>」按钮                                                                                                              | 向后翻页，显示下一页内容                                                     |
| 8    | 在分页组件中切换每页展示数量                                                                                                           | 每页展示记录数变更为切换后的数量                                             |
...
| 编号 | 步骤                                                                                                                                                                    | 预期                                                                         |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| 1    | 进入【数据资产】-【数据质量】-【脏数据管理】页面                                                                                                                        | 成功进入【脏数据管理】页面，授权的质量项目的数据源默认开启脏数据存储，无报错 |
| 2    | 点击「编辑」按钮, 「数据存储时效」改为1天后确定                                                                                                                         | 修改成功                                                                     |
| 3    | 进入「规则任务配置」页面, 点击「新建监控规则」按钮，进入监控规则配置页面                                                                                                | 成功进入监控规则配置页面，规则配置表单正常加载，无报错                       |
...
| 3    | 进入【数据资产】-【平台管理】-【数据源管理】页面，等待页面加载完成                                          | 成功进入【数据源管理】页面，数据源列表正常加载，无报错；资产平台自动引入离线平台中的带 Meta 标识的 Doris 3.x 数据源，并且已授权给质量项目                                                                              |
| 4    | 进入【数据资产】-【元数据】-【元数据同步】页面，等待页面加载完成                                            | 成功进入【元数据同步】页面，同步任务列表正常加载，无报错；自动为Doris 3.x数据源新增了一条周期同步任务，并且自动「立即同步」一次；周期同步任务配置为：数据库: doris_quality_project，数据表: 「全部」，调度周期为「周」 |
...
```
1、「公共管理-数据源中心」已存在 Doris 3.x 数据源 doris_public_ds
2、「离线开发」已存在 Meta 标识的 Doris 3.x 数据源 doris_meta_ds
3、使用 admin 账号登录系统
```
...
| 编号 | 步骤                                                               | 预期                                                               |
| ---- | ------------------------------------------------------------------ | ------------------------------------------------------------------ |
| 1    | 进入【数据资产】-【平台管理】-【数据源管理】页面，等待页面加载完成 | 成功进入【数据源管理】页面，数据源列表正常加载，无报错             |
| 2    | 点击「引入数据源」                                                 | 弹出数据源选择弹窗，显示平台中所有还未被引入至资产平台的数据源列表 |
| 3    | 选择 doris_public_ds 数据源后引入                                  | doris_public_ds 成功引入至资产平台，数据源列表中出现该记录         |
| 4    | 点击「质量项目授权」, 选择质量项目后确定                           | 1) 质量项目授权成功 2) 资产平台其他模块都可以加载到该数据源        |
| 5    | 选择 doris_meta_ds 数据源后引入                                    | doris_meta_ds 成功引入至资产平台，数据源列表中出现该记录           |
