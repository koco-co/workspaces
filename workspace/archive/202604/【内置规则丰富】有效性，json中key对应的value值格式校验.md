...
| 编号 | 步骤                                                                                                                                                                       | 预期                                                                    |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| 1    | 进入【数据质量 → 规则集管理】页面，等待规则集列表加载完成                                                                                                                  |                                                                         |
...
| ---- | -------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| 1    | 进入【数据质量 → 规则集管理】页面，等待规则集列表加载完成                                                      |                                                                                           |
| 2    | 找到规则集"rule_set_hover_test"，点击操作列的【编辑】按钮，进入 Step 2 监控规则页面，查看"悬浮展示测试包"中已配置的「格式-json格式校验」规则行的"校验key"列 |                                                                                           |
...
| ---- | ----------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | 进入【数据质量 → 规则集管理】页面，等待规则集列表加载完成                                 |                                                                                                                                                                                    |
| 2    | 找到规则集"rule_set_param_display_test"，点击操作列的【编辑】按钮，进入 Step 2 监控规则页面，查看"参数展示测试包"中已配置的「格式-json格式校验」规则行的参数展示区域 | 规则配置参数展示区域各字段内容如下：<br>1) 规则类型=「字段级」<br>2) 字段=「device_info」<br>3) 统计规则=「格式-json校验」<br>4) 校验key=「device-type」<br>5) 强弱规则=「强规则」 |
...
| 2    | 找到「json格式校验任务\_P0」，点击操作列的【立即执行】按钮，等待任务状态变为「已完成」（最长等待30秒）                                                                                                                                        | 任务执行完成：<br>1) 状态列显示「已完成」<br>2) 最新校验结果列显示「校验通过」                                                                                                                                                                                                               |
| 3    | 点击「json格式校验任务\_P0」行的【查看报告】入口，进入质量报告页面，找到「格式-json格式校验」规则对应行，查看各列内容                                                                                                                                           | 质量报告中该规则行展示：<br>1) 规则类型列=「有效性校验」<br>2) 规则名称列=「格式-json格式校验」<br>3) 字段类型列=「json」<br>4) 质检结果列=「校验通过」<br>5) 未通过原因列=「--」<br>6) 详情说明列=「符合规则key为"person-name;person-age"时的value格式要求」<br>7) 操作列无「查看详情」链接 |
...
| ---- | ----------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | 进入【数据质量 → 规则任务管理】页面，等待规则任务列表加载完成                                                     | 规则任务列表正常加载，页面无报错                                                                                                                                                                                                                                                                                              |
| 2    | 找到「json格式校验任务\_不通过」，点击【查看报告】入口，进入质量报告页面，查看「格式-json格式校验」规则行各列内容 | 质量报告中该规则行展示：<br>1) 规则类型列=「有效性校验」<br>2) 规则名称列=「格式-json格式校验」<br>3) 字段类型列=「json」<br>4) 质检结果列=「校验不通过」<br>5) 未通过原因列=「key对应value格式校验未通过」<br>6) 详情说明列=「不符合规则key为"order-amount;order-status"时的value格式要求」<br>7) 操作列显示「查看详情」链接 |
...
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | 进入【数据质量 → 规则任务管理】页面，等待规则任务列表加载完成                                                                                                                           | 规则任务列表正常加载，页面无报错                                                                                                                                                             |
| 2    | 找到「大数据量key校验任务」，点击【立即执行】按钮，等待任务状态变为「已完成」（最长等待60秒）                                                                         | 任务执行完成，状态列显示「已完成」，质检结果列正常展示校验结果                                                                                                                               |
...
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| 1    | 进入【数据质量 → 规则任务管理】页面，等待规则任务列表加载完成                                                                                                                                                                                 | 规则任务列表正常加载，页面无报错                                                         |
| 2    | 找到「抽样校验测试任务」，点击【立即执行】按钮，等待任务执行完成（最长等待60秒）                                                                                            | 任务执行完成，状态列显示「已完成」，质检结果正常展示（根据抽样到的数据判定通过或不通过） |
| 3    | 进入质量报告，查看「格式-json格式校验」规则行的详情说明列                                                                                                                                                                                     | 详情说明列展示校验信息，质量报告中显示本次校验基于抽样数据执行                           |
...
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- |
| 1    | 进入【数据质量 → 规则任务管理】页面，等待规则任务列表加载完成                                                                                                                                                                              | 规则任务列表正常加载，页面无报错                                                                                                  |
| 2    | 找到「分区校验测试任务」，点击【立即执行】按钮，等待任务执行完成（最长等待120秒）                                                                                        | 1) 任务执行完成，状态列显示「已完成」<br>2) 质检结果列显示「校验不通过」（仅校验 dt='2026-04-01' 分区，id=2的记录不符合格式要求） |
| 3    | 进入质量报告，点击「格式-json格式校验」规则行的【查看详情】，查看明细数据                                                                                                                                                                  | 1) 明细列表仅显示 dt='2026-04-01' 分区内不符合规则的记录（id=2）<br>2) dt='2026-04-02' 分区的数据（id=3、id=4）不在明细中         |
