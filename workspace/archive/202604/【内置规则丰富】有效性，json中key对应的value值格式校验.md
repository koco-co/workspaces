...
| 7 | 在规则任务列表中找到「json格式校验任务\_P0」，点击操作列的【立即执行】按钮 | 页面提示“操作成功，稍后可在任务查询中查看详情” |
...
| 7 | 在规则任务列表中找到「json格式校验任务\_不通过」，点击操作列的【立即执行】按钮 | 页面提示“操作成功，稍后可在任务查询中查看详情” |
...
| 2 | 找到「Doris3兼容性测试任务」，点击【立即执行】按钮 | 页面提示“操作成功，稍后可在任务查询中查看详情” |
...
| ---- | ---- | ---- |
| 1 | 进入【数据质量 → 规则任务管理】页面，等待规则任务列表加载完成 | 规则任务管理页面正常打开，列表加载完成 |
| 2 | 找到「Spark2兼容性测试任务」，点击【立即执行】按钮 | 页面提示“操作成功，稍后可在任务查询中查看详情” |
...
| ---- | ---- | ---- |
| 1 | 进入【数据质量 → 规则任务管理】页面，等待规则任务列表加载完成 | 规则任务管理页面正常打开，列表加载完成 |
| 2 | 找到「Hive2兼容性测试任务」，点击【立即执行】按钮 | 页面提示“操作成功，稍后可在任务查询中查看详情” |
...
```
1) 使用 admin 账号登录系统
2) 已在「通用配置 → json格式校验管理」中维护 key路径「tag-code」，中文名称「标签编码」，value格式正则：^TAG\d{4}$
3) 已执行以下 SQL 创建测试表并灌入包含不合规数据的数据：
   ...
   INSERT INTO quality_test_db.json_weak_test VALUES
     (1, '{"tag":{"code":"TAG0001"}}'),
     (2, '{"tag":{"code":"invalid"}}');
4) 已通过【数据质量 → 规则集管理】页面完成规则集"rule_set_weak_test"创建：Step 1 基础信息中关联 Doris 数据源 quality_test_db.json_weak_test 表，并新增规则包"弱规则测试包"；点击【下一步】进入 Step 2 监控规则，依次保存以下规则：<br>   - 一条强规则：完整性校验（期望通过）<br>   - 一条弱规则：「格式-json格式校验」（字段=tag_info、校验key=tag-code、强弱规则=弱规则、过滤条件=无、规则描述=无）<br>   点击页面底部【保存】，规则集创建成功
5) 已通过【数据质量 → 规则任务管理】页面完成任务"弱规则测试任务"创建：Step 1 基础信息中规则名称=弱规则测试任务，并关联同一 Doris 表；点击【下一步】进入 Step 2 监控规则，仅通过【导入规则包】导入规则集"rule_set_weak_test"的"弱规则测试包"；点击【下一步】进入 Step 3 调度属性，点击【保存】，任务创建成功
```
...
| ---- | ---- | ---- |
| 1 | 进入【数据质量 → 规则任务管理】页面，等待规则任务列表加载完成 | 规则任务管理页面正常打开，列表加载完成 |
| 2 | 找到规则集"rule_set_weak_test"，点击操作列的【编辑】按钮，进入 Step 2 监控规则页面，在"弱规则测试包"中找到「格式-json格式校验」规则行，确认「强弱规则」参数已设置为「弱规则」 | 规则配置页面正常打开，「格式-json格式校验」规则行的「强弱规则」列显示「弱规则」 |
| 3 | 返回规则任务列表，找到「弱规则测试任务」，点击【立即执行】按钮 | 页面提示“操作成功，稍后可在任务查询中查看详情” |
| 4 | 进入【数据质量 → 校验结果查询】页面，找到「弱规则测试任务」最新实例记录并打开实例详情 | 1) 本次执行生成新的实例记录，任务名称与执行时间与本次操作匹配<br>2) 实例状态显示「已完成」，整体校验结果显示「校验通过」<br>3) 实例详情中「格式-json格式校验」规则行的质检结果显示「校验不通过」，强弱规则列显示「弱规则」<br>4) 任务整体状态未因该弱规则未通过而标记为「失败」 |
...
| ---- | ---- | ---- |
| 1 | 进入【数据质量 → 规则任务管理】页面，等待规则任务列表加载完成 | 规则任务管理页面正常打开，列表加载完成 |
| 2 | 找到「大数据量key校验任务」，点击【立即执行】按钮 | 页面提示“操作成功，稍后可在任务查询中查看详情” |
...
| 3 | 关闭弹窗，返回规则任务列表，找到「key删除预览测试任务」，点击【立即执行】按钮 | 页面提示“操作成功，稍后可在任务查询中查看详情” |
...
| ---- | ---- | ---- |
| 1 | 进入【数据质量 → 规则任务管理】页面，等待规则任务列表加载完成 | 规则任务列表正常加载 |
| 2 | 找到「抽样校验测试任务」，点击【立即执行】按钮 | 页面提示“操作成功，稍后可在任务查询中查看详情” |
...
| ---- | ---- | ---- |
| 1 | 进入【数据质量 → 规则任务管理】页面，等待规则任务列表加载完成 | 规则任务列表正常加载 |
| 2 | 找到「分区校验测试任务」，点击【立即执行】按钮 | 页面提示“操作成功，稍后可在任务查询中查看详情” |
...
5) 已通过【数据质量 → 规则任务管理】页面完成任务"下载明细测试任务"创建：Step 1 基础信息中规则名称=下载明细测试任务，并关联同一 Doris 表；点击【下一步】进入 Step 2 监控规则，仅通过【导入规则包】导入规则集"rule_set_dl_test"的"下载明细测试包"；点击【下一步】进入 Step 3 调度属性并点击【保存】；随后在任务列表点击【立即执行】，页面提示“操作成功，稍后可在任务查询中查看详情”，且【数据质量 → 校验结果查询】页面已生成该任务最新实例记录，最新校验结果为「校验不通过」
```
...
5) 已通过【数据质量 → 规则任务管理】页面完成任务"通过场景测试任务"创建：Step 1 基础信息中规则名称=通过场景测试任务，并关联同一 Doris 表；点击【下一步】进入 Step 2 监控规则，仅通过【导入规则包】导入规则集"rule_set_pass_test"的"通过场景测试包"；点击【下一步】进入 Step 3 调度属性并点击【保存】；随后在任务列表点击【立即执行】，页面提示“操作成功，稍后可在任务查询中查看详情”，且【数据质量 → 校验结果查询】页面已生成该任务最新实例记录，最新校验结果为「校验通过」
```
...
5) 已通过【数据质量 → 规则任务管理】页面完成任务"报告通过展示任务"创建：Step 1 基础信息中规则名称=报告通过展示任务，并关联同一 Doris 表；点击【下一步】进入 Step 2 监控规则，仅通过【导入规则包】导入规则集"rule_set_report_pass_test"的"报告通过测试包"；点击【下一步】进入 Step 3 调度属性并点击【保存】；随后在任务列表点击【立即执行】，页面提示“操作成功，稍后可在任务查询中查看详情”，且【数据质量 → 校验结果查询】页面已生成该任务最新实例记录，最新校验结果为「校验通过」
```
...
5) 已通过【数据质量 → 规则任务管理】页面完成任务"报告不通过展示任务"创建：Step 1 基础信息中规则名称=报告不通过展示任务，并关联同一 Doris 表；点击【下一步】进入 Step 2 监控规则，仅通过【导入规则包】导入规则集"rule_set_report_fail_test"的"报告不通过测试包"；点击【下一步】进入 Step 3 调度属性并点击【保存】；随后在任务列表点击【立即执行】，页面提示“操作成功，稍后可在任务查询中查看详情”，且【数据质量 → 校验结果查询】页面已生成该任务最新实例记录，最新校验结果为「校验不通过」
```
