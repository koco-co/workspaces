...
origin: csv
---
> 来源：zentao-cases/dtstack-platform/离线开发/archive-cases/v6.4.4/【产品交付】数据同步源端_目标端支持ob for oracle.csv
> 用例数：46
...
##### 【P2】验证【数据同步任务】调度功能运行正常(Hive > OBOracle)
...
##### 【P1】验证【数据同步任务】调度功能运行正常(OBOracle > Hive)
...
##### 【P2】验证【数据同步任务】脚本模式发布正常(OBOracle > Hive)
...
##### 【P2】验证【多数据同步任务】同时发布功能正常(OBOracle > Hive)
...
##### 【P2】验证【数据同步任务+多表】同时发布功能正常(OBOracle > Hive)
...
##### 【P2】验证【数据同步任务】发布功能正常(自定义SQL, OBOracle > Hive)
...
##### 【P2】验证【数据同步任务】一键发布&导入导出发布功能正常(OBOracle > Hive)
...
##### 【P1】验证【数据同步任务】一键发布&导入导出发布功能正常(Hive > OBOracle)
...
##### 【P2】验证数据同步任务数据开发全流程(OBOracle > OBOracle)
...
##### 【P2】验证数据同步任务数据开发流程组合（Hive > OBOracle）
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【数据开发模块】页面，进入周期任务菜单页--新建向导模式的数据同步任务 | 系统提示新建成功，列表顶部出现新建的记录，记录内容与填写一致 |
...
##### 【P2】验证「增量模式-自定义SQL」数据同步功能正常(Hive > OBOracle)
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【数据开发模块】页面，新建向导模式、有增量标识的数据同步任务 | 系统提示新建成功，列表顶部出现新建的记录，记录内容与填写一致 |
...
##### 【P2】验证【增量模式-选择库表】数据同步功能正常(OBOracle > Hive)
...
##### 【P2】验证配置OBOracle作为来源及目标数据源的任务，删除任务后还原配置依然存在
...
##### 【P1】验证【通道控制-脏数据管理】功能正常(Hive > OBOracle)
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【数据开发-周期任务】页面, 创建数据同步任务 Hive2OBOracle | 系统提示创建成功，列表中出现新创建的记录，记录内容与填写一致 |
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【数据开发-周期任务】页面, 创建数据同步任务 OBOracle2OBOracle | 系统提示创建成功，列表中出现新创建的记录，记录内容与填写一致 |
...
##### 【P2】验证【字段映射】变更字段类型并刷新后, 数据同步功能正常(Hive > OBOracle)
...
##### 【P2】验证【字段映射】源表/目标表字段数量不一致, 数据同步功能正常(OBOracle > Hive)
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【数据开发-周期任务】页面, 创建数据同步任务 OBOracle2Hive | 系统提示创建成功，列表中出现新创建的记录，记录内容与填写一致 |
...
##### 【P2】验证【字段映射】添加常量字段后, 数据同步功能正常(OBOracle > OBOracle)
...
##### 【P2】验证【选择目标-主键冲突】选择Update正常生效(Hive > OBOracle)
...
##### 【P2】验证【选择目标-主键冲突】选择Insert正常生效(Hive > OBOracle)
...
##### 【P2】验证【存在空值】数据同步功能正常(OBOracle > OBOracle)
...
##### 【P2】验证【存在NULL值】数据同步功能正常(OBOracle > OBOracle)
...
##### 【P1】验证「数据来源-自定义SQL」数据同步功能正常(OBOracle(非Meta) > Hive)
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【数据开发-手动任务】页面, 创建数据同步任务 OBOracle2Hive | 系统提示创建成功，列表中出现新创建的记录，记录内容与填写一致 |
...
##### 【P2】验证未在任务中使用的OBOracle数据源取消引入(取消成功)
...
##### 【P2】验证已经在任务中使用的OBOracle数据源取消引入(取消失败)
...
##### 【P1】验证数据源中心修改OBOracle配置（e.g. jdbcUrl、描述等）同步至离线
