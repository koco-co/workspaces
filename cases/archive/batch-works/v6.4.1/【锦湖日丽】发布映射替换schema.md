---
suite_name: 【锦湖日丽】发布映射替换schema v6.4.1
description: 【锦湖日丽】发布映射替换schema v6.4.1
prd_version: v6.4.1
prd_path: ""
product: batch-works
tags:
...
origin: csv
---
> 来源：zentao-cases/dtstack-platform/离线开发/archive-cases/v6.4.1/【锦湖日丽】发布映射替换schema.csv
> 用例数：23
...
##### 【P2】验证配置了发布目标和schema映射，任务导入导出发布功能
...
----
1、“项目A”绑定“项目B”为发布目标
项目A的非metadata数据源doris1，映射配置项目B的meta数据源dorisB。不是同一套doris
schemaA、B下有对应的表
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【离线开发-项目A】页面，创建doris2doris数据同步任务：source：doris1-schema1-table1sink:doris1-scheam2-table2保存提交任务 | 系统提示提交成功，记录状态更新，列表中可见该条记录 |
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【离线开发-项目A】页面，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:dorisA-scheamB-table2保存提交任务 | 成功进入离线开发-项目A，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:dorisA-scheamB-table2保存提交任务页面，页面内容正常加载显示，无报错 |
...
| 9 | 创建表后，临时运行A、B，查看结果 | 运行成功，写入数据正确，内容与预期完全一致，无异常或错误 |

##### 【P2】验证配置schema映射，多对一，任务一键发布功能
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【离线开发-项目A】页面，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:doris1-scheam1-table2保存提交任务 | 系统提示提交成功，记录状态更新，列表中可见该条记录 |
| 2 | 进入发布任务页面，打包doris2doris | 打包成功，系统给出成功反馈，相关页面/数据状态更新为最新 |
...
##### 【P2】验证未配置schema映射，show.all.schema为false时，任务一键发布功能
...
##### 【P1】验证未配置schema映射，show.all.schema为true时，任务一键发布功能
...
##### 【P1】验证映射数据源为不同一doris集群，多任务一键发布功能
...
##### 【P2】验证映射数据源为同一doris集群，多任务一键发布功能
...
##### 【P2】验证映射数据源为不同一doris集群，配置schema映射，多对一，任务一键发布功能
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【离线开发-项目A】页面，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:dorisA-scheamB-table2保存提交任务 | 系统提示提交成功，记录状态更新，列表中可见该条记录 |
| 2 | 进入发布任务页面，打包doris2doris | 打包成功，系统给出成功反馈，相关页面/数据状态更新为最新 |
| 3 | 进入发布至目标项目页面，点击发布 | 弹出校验弹窗，校验都通过 |
| 4 | 点击确定 | 系统提示发布成功，发布状态更新，相关页面可访问 |
...
##### 【P2】验证映射数据源为同一doris集群，配置schema映射，多对一，任务一键发布功能
...
##### 【P2】验证映射数据源为不同一doris集群，配置schema映射，一对一，任务一键发布功能
...
##### 【P1】验证映射数据源为同一doris集群，配置schema映射，一对一，任务一键发布功能
...
##### 【P2】验证映射数据源为不同doris集群，未配置schema映射，任务一键发布功能
...
3、meta数据源dorisA的映射数据源为非meta数据源doris1，不同集群
doris1下存在schemaA，不存在schemaB
```
...
##### 【P2】验证映射数据源为同一doris集群，未配置schema映射，show.all.schema为false时，任务一键发布功能
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【离线开发-项目A】页面，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:dorisA-scheamA-table2保存提交任务 | 系统提示提交成功，记录状态更新，列表中可见该条记录 |
| 2 | 进入发布任务页面，打包doris2doris | 打包成功，系统给出成功反馈，相关页面/数据状态更新为最新 |
| 3 | 进入发布至目标项目页面，点击发布 | 弹出校验弹窗，校验都通过 |
| 4 | 点击确定 | 系统提示发布成功，发布状态更新，相关页面可访问 |
...
##### 【P1】验证映射数据源为同一doris集群，未配置schema映射，show.all.schema为true时，任务一键发布功能
...
| --- | --- | --- |
| 1 | 进入【离线开发-项目A】页面，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:dorisA-scheamB-table2保存提交任务 | 成功进入离线开发-项目A，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:dorisA-scheamB-table2保存提交任务页面，页面内容正常加载显示，无报错 |
| 2 | 1）进入离线开发-项目A-数据源，点击doris1的映射配置按钮2）发布目标选择doris2，查看目标项目schema下拉框 | 1）弹出映射配置弹窗2）返回doris2数据源下的所有schema |
| 3 | 添加多行配置：schemaA-schemaAschemaB-schemaBschemC-schemaCschemaD-schemaDschemaE-schemaE | 添加成功，每一行后边都有增加和删除按钮 |
| 4 | 删除第一行 | 删除成功：schemaB-schemaBschemC-schemaCschemaD-schemaDschemaE-schemaE |
| 5 | 删除第二行 | 删除成功：schemaB-schemaBschemaD-schemaDschemaE-schemaE |
| 6 | 删除最后一行 | 删除成功：schemaB-schemaBschemaD-schemaD |
| 7 | 删除所有 | 允许删除 |

##### 【P1】验证配置多行schema功能是否正确
...
| 1 | 进入【离线开发-项目A】页面，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:dorisA-scheamB-table2保存提交任务 | 成功进入离线开发-项目A，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:dorisA-scheamB-table2保存提交任务页面，页面内容正常加载显示，无报错 |
| 2 | 1）进入离线开发-项目A-数据源，点击doris1的映射配置按钮2）发布目标选择doris2，查看目标项目schema下拉框 | 1）弹出映射配置弹窗2）返回doris2数据源下的所有schema |
| 3 | 本项目schema选择schemaA，目标项目schema选择schemaA | 选择项已高亮/回显选中状态，相关联动字段随之更新 |
| 4 | 点击+按钮 | 下方添加一行schema配置 |
| 5 | 查看新增行的本项目schema和目标项目schema | 1）本项目schema中，schemaA不允许选择2）目标项目schema中，所有schema都允许选择 |
| 6 | 选择schemaB-schemaA | 选择项已高亮/回显选中状态，相关联动字段随之更新 |
| 7 | 点击+按钮，查看新增行的本项目schema和目标项目schema | 1）本项目schema中，schemaA、schemaB不允许选择2）目标项目schema中，所有schema都允许选择 |
| 8 | 配置多行，点击确定 | 系统提示保存成功，修改内容已生效并在页面中更新显示 |
| 9 | 点击映射配置，查看展示 | 回填数据正确，内容与预期完全一致，无异常或错误 |
| 10 | 上限20 |  |

##### 【P2】验证配置一行schema功能是否正确
...
| 1 | 进入【离线开发-项目A】页面，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:dorisA-scheamB-table2保存提交任务 | 成功进入离线开发-项目A，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:dorisA-scheamB-table2保存提交任务页面，页面内容正常加载显示，无报错 |
| 2 | 1）进入离线开发-项目A-数据源，点击doris1的映射配置按钮2）发布目标选择doris2，查看目标项目schema下拉框 | 1）弹出映射配置弹窗2）返回doris2数据源下的所有schema |
| 3 | 本项目schema选择schemaA，目标项目schema选择schemaA，点击确定 | 系统提示保存成功，修改内容已生效并在页面中更新显示 |
| 4 | 点击映射配置查看展示 | 回填正确，内容与预期完全一致，无异常或错误 |
| 5 | 目标项目schema选择schemaB，点击确定 | 系统提示保存成功，修改内容已生效并在页面中更新显示 |

##### 【P1】验证配置项default.data.source.show.all.schema为false时，meta数据源-schema下拉框数据是否正确
...
| --- | --- | --- |
| 1 | 进入【离线开发-项目A】页面，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:dorisA-scheamB-table2保存提交任务 | 成功进入离线开发-项目A，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:dorisA-scheamB-table2保存提交任务页面，页面内容正常加载显示，无报错 |
| 2 | 1）进入离线开发-项目A-数据源，点击dorisA1的映射配置按钮2）查看本项目schema下拉框 | 1）弹出映射配置弹窗2）返回dorisA1的默认Schema（数据源列表连接信息中的schema） |
| 3 | 点击dorisA2的映射配置按钮，查看本项目schema下拉框 | 返回dorisA2的默认Schema（数据源列表连接信息中的schema） |
| 4 | 点击dorisA3的映射配置按钮，查看本项目schema下拉框 | 展示该数据源下的所有schema |
| 5 | 发布目标选择meta数据源dorisB1，查看目标项目schema下拉框 | 返回dorisB1的默认Schema（数据源列表连接信息中的schema） |
| 6 | 发布目标选择meta数据源dorisB2，查看目标项目schema下拉框 | 返回dorisB2的默认Schema（数据源列表连接信息中的schema） |
| 7 | 发布目标选择meta数据源dorisB3，查看目标项目schema下拉框 | 展示该数据源下的所有schema |

##### 【P2】验证配置项default.data.source.show.all.schema为true时，meta数据源-schema下拉框数据是否正确

> 前置条件
```
1、“项目A”绑定“项目B”为发布目标
2、项目A、B对接三个doris集群，初始化方式分别为：创建、对接已有doris schema、不创建且不对接schema。分别为dorisA1、dorisA2、dorisA3；
dorisB1、dorisB2、dorisB3
```
...
| --- | --- | --- |
| 1 | 进入【离线开发-项目A】页面，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:dorisA-scheamB-table2保存提交任务 | 成功进入离线开发-项目A，创建doris2doris数据同步任务：source：dorisA-schemaA-table1sink:dorisA-scheamB-table2保存提交任务页面，页面内容正常加载显示，无报错 |
| 2 | 1）进入离线开发-项目A-数据源，点击dorisA1的映射配置按钮2）查看本项目schema下拉框 | 1）弹出映射配置弹窗2）返回dorisA1数据源下的所有schema |
| 3 | 点击dorisA2的映射配置按钮，查看本项目schema下拉框 | 返回dorisA2数据源下的所有schema |
| 4 | 点击dorisA3的映射配置按钮，查看本项目schema下拉框 | 返回dorisA3数据源下的所有schema |
| 5 | 发布目标选择meta数据源dorisB1，查看目标项目schema下拉框 | 返回dorisB1数据源下的所有schema |
| 6 | 发布目标选择meta数据源dorisB2，查看目标项目schema下拉框 | 返回dorisB2数据源下的所有schema |
| 7 | 发布目标选择meta数据源dorisB3，查看目标项目schema下拉框 | 返回dorisB3数据源下的所有schema |

##### 【P2】验证非meta数据源-schema下拉框数据是否正确
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【离线开发-项目A-数据源】页面，点击非meta数据源doris1的映射配置按钮 | 1）弹出映射配置弹窗 |
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【离线开发-项目A-数据源】页面，点击数据源doris的映射配置按钮 | 1）弹出映射配置弹窗：下方新增Schema映射2）schema映射列表展示暂无数据，有本项目schema、目标项目schema、操作三个字段，和新增按钮 |
