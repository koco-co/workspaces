---
suite_name: OceanBase计算引擎适配 v6.3.0
...
| 7 | 查看该任务 | 该任务的来源数据源与目标数据源均替换为与测试环境配置映射的OceanBase for MySQL/OceanBase for Oracle的数据源 |
...
| 3 | 选择来源数据源，点击下一步 | 进入选择目标页面 |
| 4 | 点击数据同步目标下拉框 | 来源类型不支持OceanBase for MySQL/OceanBase for Oracle类型，选择其他任意数据库 |
| 5 | 选择OceanBase for MySQL/OceanBase for Oracle，点击数据库下拉框 | 可选范围为数据库连接用户下的所有数据库 |
...
| 5 | 选择OceanBase for MySQL/OceanBase for Oracle数据源 | 显示OceanBase for MySQL/OceanBase for Oracle下的schema |
...
| 5 | 点击OceanBase for MySQL/OceanBase for Oracle的schema | 显示schema下数据地图中存在的表，数据正确，内容与预期完全一致，无异常或错误 |
...
| 8 | 建立OceanBase for MySQL/OceanBase for Oracle任务，执行obop FUNCTION aa.getCity(); | return query select * from city; |
| 9 | 去OceanBase for MySQL/OceanBase for Oracle底层数据库查看函数被删除 | end; |
| 10 |  | $$ |
| 11 |  | language plpgsql; |
...
| 8 | 建立OceanBase for MySQL/OceanBase for Oracle任务，执行delete * from zy_1130 where name = “aa“.“test2“('a','b'); | $BODY$ |
...
| 6 | 其他信息正确填写，点击确定 | RETURNS “pg_catalog“.“varchar“ AS $BODY$ SELECT |
| 7 | 点击自定义函数，弹出函数详情弹窗 | concat ( $1,$2 ); |
| 8 | 建立OceanBase for MySQL/OceanBase for Oracle任务，执行update zy_1130 set age = age + 1 where name = “aa“.“test2“('a','b'); | $BODY$ |
| 9 |  | LANGUAGE sql VOLATILE |
| 10 |  | COST 100 |
| 11 |  | 关闭弹窗后，弹出toast提示：创建成功，系统给出创建成功提示，列表中出现新创建的记录 |
| 12 |  | 正确回显函数数据 |
...
| 6 | 其他信息正确填写，点击确定 | $$ |
| 7 | 点击自定义函数，弹出函数详情弹窗 | begin |
| 8 | 建立OceanBase for MySQL/OceanBase for Oracle任务，执行select getCity(); | return query select * from city; |
| 9 |  | end; |
| 10 |  | $$ |
| 11 |  | language plpgsql; |
| 12 |  | 关闭弹窗后，弹出toast提示：创建成功，系统给出创建成功提示，列表中出现新创建的记录 |
...
| 9 | 建立OceanBase for MySQL/OceanBase for OracleSQL任务，执行select test2('zhang','san') from people;且test2自定义函数高亮显示 | LANGUAGE sql VOLATILE |
| 10 |  | COST 100 |
| 11 |  | 关闭弹窗后，弹出toast提示：创建成功，系统给出创建成功提示，列表中出现新创建的记录 |
| 12 |  | 正确回显函数数据 |
| 13 |  | 插入新增自定义函数 |
...
| 3 | 选择‘手动依赖’，点击‘添加依赖’ | 跳转到‘添加依赖界面’ |
| 4 | 勾选依赖推荐 | 列表展示为： |
| 5 | 勾选任务‘putong_OceanBase for MySQL/OceanBase for OracleSQL’、putong_OceanBase for MySQL/OceanBase for OracleSQL，点击保存 | 任务                  类型         项目   租户                 产品        责任人  关联内容 |
| 6 |  | putong_OceanBase for MySQL/OceanBase for OracleSQL  OceanBase for MySQL/OceanBase for OracleSQL  putong   DTstack租户  离线开发  putong  chema.hive_table |
| 7 |  | putong_OceanBase for MySQL/OceanBase for OracleSQL  OceanBase for MySQL/OceanBase for OracleSQL  putong   DTstack租户  离线开发  putong  chema.OceanBase for MySQL/OceanBase for Oracle_table |
...
| 5 | create table if not exists putong_table(id int,name varchar(255)); | -- type OceanBase for MySQL/OceanBase for Oracle SQL |
...
| 7 | select * from putong_table; | -- create time 创建任务时间 |
| 8 | 全选所有sql临时运行 | -- desc 任务描述 |
| 9 | 新建OceanBase for MySQL/OceanBase for OracleSQL，输入sql：select * from putong_table;临时运行 | 前端会按照分号进行划分，分别调4次临时运行的接口，临时运行成功； |
...
| 3 |  | 默认注释模版如： |
| 4 |  | -- name 任务名称 |
| 5 |  | -- type OceanBase for MySQL/OceanBase for Oracle SQL |
...
| 4 | 保存 | for mysql【创建】筛选框正常显示 |
| 5 |  | oracle对接成功，【项目设置->开发设置】中出现OceanBase for MySQL/OceanBase for Oracle计算引擎 |
| 6 |  | mysql创建成功，底层创建一个和用户名相同的schema |
