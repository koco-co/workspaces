# 【数据质量】质检式质量报告查看、下载 v6.4.3
> 来源：zentao-cases/dtstack-platform/数据资产/archive-cases/v6.4.3/【数据质量】质检式质量报告查看、下载.csv
> 用例数：8
...
| 3 | 在【已配置报告】中新建报告, 规则范围默认【全部】,报告周期为【一次性】, 并配置car_compare02表, 点击【确定】 | 1) 表单提交成功，系统给出提交成功提示，状态更新为下一处理阶段 |
| 4 | 点击【已生成报告】页签, 进入报告详情, 点击操作中的【查看日志】 | 质量报告校验失败 |
| 5 |  | 支持查看日志数据，和任务实例模块的查看日志内容保持一致 |

##### 验证「报告详情」-页面&功能正常(Doris 2.x) 「P1」
...
| 3 | UI CHECK | 显示一份质量评估汇总表格: |
| 4 | 点击质量评估汇总（car_compare01 | 质量评估汇总(car_compare01--delivery_time=2025-09-19) |
| 5 | -delivery_time）中的【车型】筛选图标 | 1) 支持选择car_config字段的所有枚举 |
| 6 | 勾选【H53a】 , 点击【确定】 | 按钮: 重置/确定, 重置默认置灰 |
| 7 | 重置后, 再次点击【车型】筛选图标，勾选【H53a】和【H53b】 , 点击【确定】 | 显示【车型】为【H53a】的记录 |
| 8 |  | 显示【车型】为【H53a】或【H53b】的记录 |

##### 验证「报告详情」-页面&功能正常(SparkThrift 2.x) 「P2」
...
| 7 |  | 若规则校验中含N个表, 则依次向下展示N个2~4中的内容 |
| 8 |  | 按钮: 下载报告, 规则校验明细中的校验失败的【查看日志】和校验未通过的【查看详情】 |
| 9 |  | 1) 报告生成时间: ${与报告记录中的【生成时间】一致} |
| 10 |  | 表名/数据源/数据库/检测数据范围: ${该质量报告关联的规则校验表/源/库/分区} |
| 11 |  | 表行数: ${当前报告生成时的表行数} |
| 12 |  | 车辆数：${当前报告生成时规则校验表中vin字段的count值}, 若没有vin字段则不展示车辆数 |
| 13 |  | 字段数：${当前报告生成时的字段数} |
| 14 |  | 校验规则数：${当前报告生成范围内的规则数量} |
| 15 |  | 校验通过率：${统计校验通过的子规则/总规则数量（总规则指的是报告范围内的规则数量）*100%，保留两位小数} |
| 16 |  | 车系/车型/动力类型: ${校验表中的car_series/car_config/car_power字段的枚举值}, 车型可支持筛选 |
| 17 |  | 车辆数: ${该分区表中同一个车系-车型-动力类型的vin字段的count值} |
| 18 |  | 表中包含车辆数: ${当前校验表里面同一个车系-车型-动力类型的vin字段去重之后计算的车辆数} |
| 19 |  | 车辆总数: ${该分区表中同一个车型的的vin字段的count值}, 且车型一致的将合并单元格统计一下 |
| 20 |  | 表中包含总车辆数: ${当前校验表里面同一个车型的vin字段去重之后计算的车辆数}, 且车型一致的将合并单元格统计一下 |
| 21 |  | 规则校验明细: ${该表在【规则任务配置】中的校验规则} |
