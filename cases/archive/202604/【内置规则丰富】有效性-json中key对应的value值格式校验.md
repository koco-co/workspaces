...
##### 【P1】验证规则集管理页按规则名称查询可命中格式-json格式校验

> 前置条件
```
1、使用账号 qa_rule_admin 登录数据资产系统，账号具备【规则集管理】权限。
2、当前环境规则集管理页的内置规则列表至少包含【格式-json格式校验】【格式校验-自定义正则】【枚举值校验】三条规则，其中【格式-json格式校验】规则分类为“有效性校验”，关联范围为“字段”。
3、规则集管理页提供【规则名称】搜索框、【规则分类】【关联范围】【规则状态】筛选项及【导出规则库】按钮。
```
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【数据质量 → 规则集管理】页面 | 规则集管理页正常加载，列表展示内置规则数据，可进行查询操作。 |
| 2 | 在【规则名称】搜索框输入 `json格式` 并点击【查询】 | 列表返回包含“json格式”关键字的规则，命中新增内置规则【格式-json格式校验】，不展示无关规则。 |
| 3 | 将关键词改为 `格式-json格式校验` 并再次点击【查询】 | 列表精确命中【格式-json格式校验】1 条记录，规则分类展示为“有效性校验”，关联范围展示为“字段”。 |
| 4 | 清空关键词后点击【查询】 | 列表恢复默认规则库结果，新增内置规则【格式-json格式校验】仍可在结果集中查看。 |

##### 【P1】验证规则集管理页按分类和关联范围筛选可定位格式-json格式校验

> 前置条件
```
1、使用账号 qa_rule_admin 登录数据资产系统，账号具备【规则集管理】权限。
2、当前环境规则集管理页的内置规则列表至少包含【格式-json格式校验】【格式校验-自定义正则】【枚举值校验】以及其它非有效性规则。
3、【格式-json格式校验】规则分类为“有效性校验”，关联范围为“字段”，当前规则状态为“启用”。
```
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【数据质量 → 规则集管理】页面 | 规则集管理页正常加载，筛选区默认为空，列表展示全部内置规则。 |
| 2 | 在筛选区选择【规则分类=有效性校验】【关联范围=字段】后点击【查询】 | 列表仅返回满足筛选条件的规则，结果中包含【格式-json格式校验】，不再展示完整性类规则。 |
| 3 | 继续选择【规则状态=启用】并点击【查询】 | 列表结果继续保持可用，仅返回状态为“启用”的有效性字段规则，【格式-json格式校验】仍在结果中。 |
| 4 | 点击【重置】后再次点击【查询】 | 所有筛选项恢复默认状态，列表重新展示完整规则库数据。 |

##### 【P1】验证规则集管理页导出规则库包含格式-json格式校验

> 前置条件
```
1、使用账号 qa_rule_admin 登录数据资产系统，账号具备【规则集管理】权限。
2、当前环境规则集管理页的内置规则列表至少包含【格式-json格式校验】【格式校验-自定义正则】【枚举值校验】三条规则。
...
| 编号 | 步骤 | 预期 |
| --- | --- | --- |
| 1 | 进入【数据质量 → 规则集管理】页面 | 规则集管理页正常加载，可查看规则列表并执行导出操作。 |
| 2 | 点击【导出规则库】按钮 | 页面弹出导出确认提示，可继续确认或取消导出。 |
| 3 | 在确认提示中先点击【取消】，再重新点击【导出规则库】并点击【确认】 | 首次取消后不产生下载文件；再次确认后浏览器开始下载规则库文件。 |
| 4 | 打开下载的规则库文件并查看导出内容 | 导出文件名以“内置规则库_”开头；文件中包含【格式-json格式校验】规则，且规则分类、关联范围、规则解释、规则描述等信息与页面展示一致。 |
