...
##### 【P1】验证规则库配置页内置规则按规则名称查询可命中格式-json格式校验

> 前置条件
```
1、使用账号 qa_rule_admin 登录数据资产系统，账号具备【规则库配置】权限。
2、当前环境【规则库配置】页面的【内置规则】列表至少包含【格式-json格式校验】【格式校验-自定义正则】【枚举值校验】三条规则，其中【格式-json格式校验】规则分类为“有效性校验”，关联范围为“字段”。
...
##### 【P1】验证规则库配置页内置规则按分类和关联范围筛选可定位格式-json格式校验

> 前置条件
```
1、使用账号 qa_rule_admin 登录数据资产系统，账号具备【规则库配置】权限。
2、当前环境【规则库配置】页面的【内置规则】列表至少包含【格式-json格式校验】【格式校验-自定义正则】【枚举值校验】以及其它非有效性规则。
...
| 2 | 在列表中使用【规则分类=有效性校验】【关联范围=字段】筛选项后触发筛选 | 列表仅返回满足筛选条件的规则，结果中包含【格式-json格式校验】，不再展示完整性类规则。 |
| 3 | 继续选择【规则状态=启用】并点击【查询】 | 列表结果继续保持可用，仅返回状态为“启用”的有效性字段规则，【格式-json格式校验】仍在结果中。 |
| 4 | 点击【重置】后再次点击【查询】 | 所有筛选项恢复默认状态，列表重新展示完整规则库数据。 |

##### 【P1】验证规则库配置页导出规则库包含格式-json格式校验

> 前置条件
```
1、使用账号 qa_rule_admin 登录数据资产系统，账号具备【规则库配置】权限。
2、当前环境【规则库配置】页面的【内置规则】列表至少包含【格式-json格式校验】【格式校验-自定义正则】【枚举值校验】三条规则。
...
| 4 | 打开下载的规则库文件并查看导出内容 | 导出文件名以“内置规则库_”开头并带当天日期，扩展名为“.xlsx”；文件中包含【格式-json格式校验】规则，且规则分类、关联范围、规则解释、规则描述等信息与页面展示一致。 |
