...
- 页面入口：规则配置页出现 `key范围校验` 选项，完整导航路径待澄清。
...
1. 本规则在系统中的完整导航路径与入口按钮文案尚未确认。
2. `字段仅支持单选` 具体限制的是校验字段选择控件还是整个规则块仅允许配置一条字段记录，蓝湖文本仍需结合页面确认。
3. 回显格式 `key1-key2;key11-key22` 中分号与层级的精确映射规则未在文字中展开。
