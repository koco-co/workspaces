...
- 页面类型：质量规则配置页，附带结果查询与质量报告说明。
- 区域构成：规则配置区、校验内容选择区、结果查询说明区、质量报告说明区、规则库说明区。
- 关键操作：选择 `key范围校验`、切换 `包含/不包含`、按层级选择 key、查看失败详情。
- 列表信息：质量报告展示 `规则类型`、`规则名称`、`字段名称`、`字段类型`、`质检结果`、`未通过原因`、`详情说明`、`操作`。
- 检索条件：校验内容选择框支持输入查询。
- 输入项：`校验方法`、`校验内容`、`过滤条件`、`强弱规则`、`规则描述`。
- 流程/状态：key 来源于通用配置；默认加载前 200 条；失败支持查看明细和日志，成功不落明细。
- 识别限制：`字段仅支持单选` 的精确约束对象以及多层级回显分隔规则无法仅凭截图确认。
...
- W001：`字段仅支持单选` 的约束口径未明确，影响异常用例的边界覆盖。
- W002：多层级回显分隔规则未明确，影响回显校验的精确预期。
